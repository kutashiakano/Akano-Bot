...
module.exports = {
    async handler(chatUpdate) {
        ...
        try {
            ...
            if (m.isGroup) {
                let member = (await store.fetchGroupMetadata(m.chat, sock)).participants.map(a => a.id);
            ...
            const participants = (m.isGroup ? (await store.groupMetadata[m.chat]).participants : []) || [];
            ...
            for (let name in global.plugin) {
                ...
                if (typeof plugins.before === "function") {
                    if (await plugins.before.call(this, m, {
                        match, sock: this, participants, groupMetadata, user, bot, isROwner, isOwner, isRAdmin, isAdmin, isBotAdmin, isPrems, isBans, chatUpdate
                    })) continue;
                }
                ...
                if (opts && match && m) {
                    ...
                    let isAccept = (prefixCommand instanceof RegExp && prefixCommand.test(command)) ||
                        (Array.isArray(prefixCommand) && prefixCommand.some(cmd => cmd instanceof RegExp ? cmd.test(command) : cmd === command)) ||
                    ...
                    if ((!db.data.users[m.sender].limit > 100) & (db.data.users[m.sender].limit < 1)) {
                        sock.sendMessage(m.chat, { text: "Your limit has been reached. Please wait until your limit is reset." }, { quoted: m });
                        continue;
                    }

                    let extra = {
                        match, usedPrefix, noPrefix, args, command, text, sock: this, participants, groupMetadata, user, bot, isROwner, isOwner, isRAdmin, isAdmin, isBotAdmin, isPrems, isBans, chatUpdate
                    };
            ...
            if (!isCommand && opts["groupOnly"] && !m.fromMe && !m.chat.endsWith("g.us")) {
                return;
            }
    ...
    async participantsUpdate({ id, author, participants, action }) {
        ...
        switch (action) {
            ...
            case "remove":
                if (action === "add") {
                    metadata.participants.push(...participants.map(sender => ({ id: jidNormalizedUser(sender), admin: null })));
                } else if (action === "remove") {
                    metadata.participants = metadata.participants.filter(p => !participants.includes(jidNormalizedUser(p.id)));
                }

                if (chat.welcome) {
                    let groupMetadata = store.groupMetadata[id] || store.fetchGroupMetadata(id, this);
