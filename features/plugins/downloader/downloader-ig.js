module.exports = {
   help: ['igdl', 'instagram', 'ig'],
   command: ['igdl', 'instagram', 'ig'],
   tags: ['downloader'],
   run: async (m, { sock, text }) => {
      const result = await scraper.Instagram(text);
      const { oversize, size } = await sock.sizeLimit(result.url[0], global.max_upload);

      if (oversize) {
         return m.reply(`File size (${size}) exceeds the maximum limit. Please download it manually: ${text}`);
      }

      const caption = `${global.settings.dot} Username: @${result.metadata.username || 'unknown'}\n` +
         ...
         (result.metadata.caption ? `${global.settings.dot} Caption: ${result.metadata.caption}\n` : '');
