...
module.exports = {
	...
	run: async (m, {
		...
		Object.entries(plugin)
			.filter(([, plugin]) => plugin.help && !(plugin.disabled || (plugin.before && !plugin.run)))
			.forEach(([, plugin]) => {
				const commandCategories = Array.isArray(plugin.category) ? plugin.category.flat().filter(Boolean) : (plugin.tags ? (Array.isArray(plugin.tags) ? plugin.tags : [plugin.tags]) : []);
				const commandHelps = Array.isArray(plugin.help) ? plugin.help.filter(Boolean) : [plugin.help];

				commandHelps.forEach(helpCmd => {
					const premiumBadge = plugin.premium ? " [PREMIUM]" : "";
					const commandEntry = plugin.customPrefix ? `${helpCmd.replace(/^\./, '')}${premiumBadge}` : `${usedPrefix}${helpCmd}${premiumBadge}`;
		...
		const categorySections = Array.from(categoryMap.entries())
			...
			.map(([category, commands]) => {
				return `┏ ⌞ ${category} ⌝\n${Array.from(commands).sort().map(cmd => `│ • ${cmd}`).join('\n')}\n╰`;
			})
			.join('\n\n');
