...
let handler = async (m, { sock, args, usedPrefix, command }) => {
  ...
  if (
    !(
      (args[0] && args[0] == "plz") ||
      (await global.sock).user.jid == sock.user.jid
    )
  ) {
    return m.reply(
      `Perintah ini hanya bisa digunakan di bot utama! wa.me/${global.sock.user.jid.split`@`[0]}?text=${usedPrefix}codebot`,
    );
  }
  ...
  async function initializeBot() {
    ...
    if (!sock.authState.creds.registered) {
      ...
      setTimeout(async () => {
        ...
        let codeBot = await sock.requestPairingCode(
          cleanedNumber,
          randomPairing,
        );
    ...
    async function connectionUpdate(update) {
      ...
      if (
        ...
        if (i < 0)
          return console.log(await setupEvents(true).catch(console.error));
