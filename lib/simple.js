...
sock.sendFile = async (jid, media, filename = "", caption = "", quoted, options = {}) => {
    ...
    let type = isBuffer ? { mime: options.mimetype || "application/octet-stream" } : await sock.getFile(media, true);
...
exports.smsg = (sock, m, hasParent) => {
    ...
    if (m.message) {
        ...
        if (
            m.chat == "status@broadcast" && ["protocolMessage", "senderKeyDistributionMessage"].includes(m.mtype)
        )
            m.chat =
