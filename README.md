...
|---|---|---|---|
| **Node.js** | `>=20` (tested on 22 / 26) | `pkg install nodejs` / `nvm` | Required |
...
- **WhatsApp:** powered by **Baileys** (`@whiskeysockets/baileys`). QR / pairing code, multi-file auth, in-memory store, anti-delete, verification, group automation.
...
|---|---|---|
| **baileys** (`@whiskeysockets/baileys`) | WhatsApp Web API | WhatsApp bot core, pairing, messaging |
...
ESM bot (`"type": "module"`, Node `>=20`). WhatsApp in-memory store persists to `store.json` and is restored on boot.

To migrate or restore, keep only:

```
system/bot/whatsapp/sessions/   # Baileys creds + store.json (persist, restored on boot)
~/.akano-env                    # DISCORD_TOKEN, TELEGRAM_TOKEN, ID_OWNER, YT_SESSION_KEY, DASHBOARD_KEY
```

Telegram: single-poller lockfile (`akano-telegram.lock` in OS tmpdir) — run Telegram on ONE instance only, or you get 409 Conflict. Set `TELEGRAM_ENABLED=false` on secondary instances.
...
Bot itself is ESM (`"type": "module"`, Node `>=20`) — use `import` in bot code, `require()` only for SDK CJS compat.
...
| `grammy/discord.js not installed` | Install parsial | `rm -rf node_modules && npm install` (semua dep platform ikut `@kutashiakanocanzy/sdk`) |
