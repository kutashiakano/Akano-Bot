...
![Node](https://img.shields.io/badge/Node.js-%3E%3D20-808080?style=flat)
...
|---|---|---|---|
| **Node.js** | `>=20` (diuji pada 22 / 26) | `pkg install nodejs` / `nvm` | Wajib |
...
- **WhatsApp:** didukung **Baileys** (`@whiskeysockets/baileys`). QR / pairing code, multi-file auth, in-memory store, anti-hapus, verifikasi, otomasi grup.
...
|---|---|---|
| **baileys** (`@whiskeysockets/baileys`) | WhatsApp Web API | Core bot WhatsApp, pairing, messaging |
...
Bot ESM (`"type": "module"`, Node `>=20`). Store in-memory WhatsApp dipersist ke `store.json` dan dipulihkan saat boot.

Untuk migrasi atau restore, simpan hanya:

```
system/bot/whatsapp/sessions/   # kreds Baileys + store.json (persist, dipulihkan saat boot)
~/.akano-env                    # DISCORD_TOKEN, TELEGRAM_TOKEN, ID_OWNER, YT_SESSION_KEY, DASHBOARD_KEY
```

Telegram: lockfile single-poller (`akano-telegram.lock` di tmpdir OS) — jalankan Telegram hanya di SATU instance, atau kena 409 Conflict. Set `TELEGRAM_ENABLED=false` di instance sekunder.
...
Setiap pesan WhatsApp masuk diserialisasi via `smsg()` / `serializeM()` dari `@kutashiakanocanzy/sdk`:
...
// Exif manual (level rendah) — via SDK, tanpa file lokal
...
// Helper converter (benar) — langsung dari SDK
...
// ESM: import { Platform } from "@kutashiakanocanzy/sdk"

const wa = new Platform({ platform: "whatsapp", plugins_dir: "./plugins", session_dir: "./session" });
wa.on("ready", x => console.log(x.message));
await wa.start();
```

Bot ini ESM (`"type": "module"`, Node `>=20`) — gunakan `import` di kode bot, `require()` hanya untuk kompat CJS SDK.
...
**Import terpadu SDK (flat root — tanpa deep path):**
...
Ekspor root SDK: `defineBot` (builder plugin bergate), `Cooldown`, `Queue`, `resize`, `texted`, `reply`, `media`, `smsg`, `AIRichBuilder`, `card`, `button`, `md`, `status`, namespace `telegram` / `discord` / `whatsapp` / `core` / `common` (daftar lengkap di README `@kutashiakanocanzy/sdk`).
...
| `pino/index.js` error | Lockfile rusak / install parsial | `rm -rf node_modules package-lock.json && npm install` (jangan pin pino manual — versi ikut SDK) |
| `grammy/discord.js tidak terinstall` | Install parsial | `rm -rf node_modules && npm install` (semua dep platform ikut `@kutashiakanocanzy/sdk`) |
