...
> [!WARNING]
> The old Google Console flows (web URL login and device code entry via `YT_OAUTH_CLIENT_ID` / `YT_OAUTH_CLIENT_SECRET` / `YT_OAUTH_FLOW`) were **removed** — delete those variables from any fork or `.env`.
...
Related: [docs/cookies.en.md](cookies.en.md) · [docs/adding-a-plugin.en.md](adding-a-plugin.en.md) · `README` → "YouTube Music — Deep Dive"