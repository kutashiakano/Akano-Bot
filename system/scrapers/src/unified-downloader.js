...
async function downloadAndSendTG(ctx, url, opts = {}) {
  ...
  try { const tgsdk = require("../../bot/sdk/telegram"); toInputFile = (p)=> tgsdk.inputFile(p); } catch { try { const g = require("grammy"); toInputFile = (p)=> new g.InputFile(p); } catch { toInputFile = ()=> { throw new Error("grammy InputFile is not available"); }; } }
