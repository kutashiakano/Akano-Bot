...
function _sdkDiscordEngine() {
  try {
    const eng = _sdkMod?.discord?.engine ?? _sdkMod?.default?.discord?.engine;
...
async function download(url, opts = {}) {
  ...
  if (!normalized.cookies) {
    const candidates = [
      path.join(import.meta.dirname, "../../../cookies.txt"),
      path.join(import.meta.dirname, "../../../../../cookies.txt"),
...
async function downloadAndSendTG(ctx, url, opts = {}) {
  ...
  try {
    if (_sdkMod) {
      const tgMediaFile = _sdkMod.mediaFile ?? _sdkMod.default?.mediaFile;
      if (typeof tgMediaFile === "function") toInputFile = (p) => tgMediaFile(p);
    }
    if (!toInputFile) {
      const g = await import("grammy");
      toInputFile = (p) => new g.InputFile(p);
    }
  } catch { toInputFile = () => { throw new Error("grammy InputFile is not available"); }; }
...
async function downloadAndSendDC(interaction, url, opts = {}) {
  ...
  try {
    ...
    if(valid.length>1){
      const Attach = interaction.client?.AttachmentBuilder || _sdkDiscordEngine().AttachmentBuilder;
...
try {
  if (global.scraper) {
    global.scraper.unified = unified;
    global.scraper["unified-downloader"] = unified;
    global.scraper.unifiedDownloader = unified;
  }
} catch {}
