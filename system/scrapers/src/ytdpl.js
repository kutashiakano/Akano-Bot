...
import { AUDIO_FILTERS } from "../../bot/discord/plugins/music/utils.js";

const AUDIO_CACHE_DIR = path.join(import.meta.dirname, "../../../tmp/audio_cache");
...
class MediaDownloader {
  constructor(config = {}) {
    ...
    this.baseDir = config.baseDir || path.join(import.meta.dirname, "../../../tmp");
  ...
  async _execute(bin, args, cwd) {
    try {
      const { code, stdout, stderr } = await (await import("../../core/heavy.js")).heavyExec(bin, args, cwd);
...
export { MediaDownloader, OPT_FLAG_MAP };
const ytdplDefault = new MediaDownloader;
ytdplDefault.MediaDownloader = MediaDownloader;
ytdplDefault.OPT_FLAG_MAP = OPT_FLAG_MAP;
export default ytdplDefault;
