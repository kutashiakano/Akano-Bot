...
class YTDL {
  ...
  static #getId(url) {
    const regex = /(?:v=|be\/|shorts\/|embed\/|v%3D|vi?\/|u\/\w\/|attribution_link\?a=.*?v%3D|^)([\w-]{11})/;
  ...
  static async #savetube(link, quality, type) {
    try {
      ...
      const { data: { data: info } } = await axios.post(
        `https://${this.#CDN}/info`,
        { url: link },
        { headers: { ...headers, authority: this.#CDN } }
      );

      const { data: { data: dl } } = await axios.post(
  ...
  static async #vreden(videoId, type) {
    try {
      const { data } = await axios.get(`https://ytdl.vreden.web.id/metadata?url=https://www.youtube.com/watch?v=${videoId}`);
      return {
