...
import { Store } from "./aesstore.js";

const DIR = path.join(import.meta.dirname, "../../../tmp/ytsession");
...
async function getSession(uid) {
  if (!live.has(uid) && hasCached(uid)) {
    const yt = await withTimeout(Innertube.create({
      retrieve_player: false,
      cache: new CacheK(uid)
    }), 1e4, "session.create");
    yt.session.on("auth", () => {
    ...
    await withTimeout(yt.session.signIn(), 1e4, "session.signIn");
...
async function profile(uid) {
  ...
  let info;
  try {
    info = await withTimeout(yt.account.getInfo(), 8e3, "profile.info");
  } catch {
    return null;
  }
  ...
  try {
    ...
    if (id && /^UC/.test(id)) {
      const ch = await withTimeout(yt.getChannel(id), 6e3, "profile.channel");
  ...
  if (music.has(uid)) {
    try {
      const lm = await withTimeout(yt.actions.execute("/browse", {
        browseId: "LM",
        client: "YTMUSIC"
      }), 6e3, "profile.likes");
      const data = lm?.data || lm;
      const ne = findVal(data, "numEntriesText");
      if (ne?.runs?.[0]?.text) likes = ne.runs[0].text; else if (ne?.text) likes = ne.text; else {
        const shelf = findVal(data, "musicPlaylistShelfRenderer");
        if (shelf?.contents?.length) likes = String(shelf.contents.length);
      }
    } catch {}
    try {
      const pl = await withTimeout(yt.actions.execute("/browse", {
        browseId: "FEmusic_liked_playlists",
        client: "YTMUSIC"
      }), 6e3, "profile.playlists");
      const data = pl?.data || pl;
      const grid = findVal(data, "musicGridRenderer");
      const items = grid?.contents || [];
      playlists = items.map(i => i?.gridPlaylistRenderer?.title?.runs?.[0]?.text || i?.musicNavigationButtonRenderer?.buttonText?.runs?.[0]?.text || "").filter(Boolean);
    } catch {}
  }
  return {
    ...
    photo: item.account_photo?.find?.(t => t?.url)?.url || item.account_photo?.contents?.find?.(t => t?.url)?.url || "",
    subs: subs,
    videos: videos,
    likes: likes,
    playlists: playlists
...
const TV_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:88.0) Gecko/20100101 Firefox/88.0";
  ...
  const send = async token => {
    const res = await fetch("https://www.youtube.com/youtubei/v1/" + endpoint, {
      ...
      headers: {
        ...
        "X-Goog-Request-Time": String(Math.floor(Date.now() / 1e3)),
        "User-Agent": TV_UA,
...
async function likes(uid, limit = 25) {
  const data = await withTimeout(tvRaw(uid, {
    context: TV_CTX,
...
async function newPl(uid, title) {
  const data = await withTimeout(tvReq(uid, "playlist/create?alt=json", {
    context: TV_CTX,
    title: String(title || "").slice(0, 60),
    description: "",
    privacyStatus: "PRIVATE"
  }), 15e3, "playlist.create").catch(() => null);
  ...
  return {
    ok: false,
    message: "Playlist creation isn't supported with the TV sign-in used by this bot. You can still add songs to existing playlists."
  };
}
...
async function moods(limit = 40) {
  ...
  const seen = new Set;
  return out.filter(o => seen.has(o.browseId + "|" + o.params) ? false : (seen.add(o.browseId + "|" + o.params),
  true)).slice(0, limit);
}
...
export { login, out, getSession, profile, getPend, clearPend, has, likes, plists, plist, like, addPl, newPl, charts, moods, moodPls, radio };
export default {
  login: login,
  out: out,
