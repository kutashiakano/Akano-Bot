...
async function _getClient(session) {
  ...
  _connecting = _requireInnertube().create({
...
async function sSongs(query, limit = 10, session = null) {
  ...
  for (const client of attempts) {
    try {
      ...
      const shelves = Object.values(search.contents || {}).filter(x => x?.type === "MusicShelf");
      const items = [];
...
async function getRelated(videoId, limit = 10, session = null, enrich = true) {
  ...
  const sections = Array.isArray(related?.contents) ? related.contents : Object.values(related?.contents || {});
  const items = [];
  ...
  const enriched = await Promise.all(items.map(async song => {
    try {
      ...
      return {
        ...song,
        duration: basic.duration || song.duration
      };
    } catch (e) {
      return song;
    }
...
async function searchSuggestions(query, limit = 8) {
  try {
    ...
    return list.filter(s => s?.text || s?.title?.text).map(s => s.text || s.title.text).slice(0, limit);
  } catch (e) {
    return [];
  }
}
...
function _map(node) {
  if (!node) return null;
  const id = node.id || node.playlistId || node.browseId || node.endpoint?.payload?.browseId || node.endpoint?.payload?.playlistId || "";
  if (!id) return null;
  const item_type = node.item_type || "song";
  const title = node.title?.text || node.title || "";
  const artist = node.artist?.name || node.artists?.[0]?.name || node.subtitle?.text || "Unknown";
  return {
    id: id,
    type: item_type,
    name: title,
    ...
    thumbnail: node.thumbnails?.[0]?.url || node.thumbnail?.contents?.find(t => t?.url)?.url || ""
  };
}
...
function _cols(r) {
  return (r.flexColumns || []).map(c => (c.musicResponsiveListItemFlexColumnRenderer?.text?.runs || []).map(x => x.text || "").join("")).filter(Boolean);
}
...
function _fromRaw(node) {
  ...
  const cols = _cols(r);
  const { title: title, artist: artist, duration: duration } = _meta(cols);
  const thumbs = r.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails || [];
  let type = "song";
  if (ep.browseEndpoint) {
    const id = ep.browseEndpoint.browseId || "";
    if (id.startsWith("MPRE")) type = "album"; else if (id.startsWith("VL")) type = "playlist"; else if (id.startsWith("UC")) type = "artist";
  }
  const id = r.playlistItemData?.videoId || ep.watchEndpoint?.videoId || ep.browseEndpoint?.browseId || ep.playlistEditEndpoint?.playlistId || "";
  if (!id || !title) return null;
  const badges = (r.badges || []).map(b => b.musicInlineBadgeRenderer?.label?.runs?.[0]?.text || "").join(" ");
...
async function typeParams(session) {
  ...
  _walkRaw(res?.data, node => {
    ...
    if (!chip) return;
    const label = (chip.text?.runs || []).map(x => x.text || "").join("").toLowerCase().replace(/s$/, "");
    const params = chip.navigationEndpoint?.searchEndpoint?.params;
    if (!params) return;
    if (label === "song" || label === "video" || label === "album") map[label] = params; else if (label === "profile" || label === "profiles" || label === "artist") map.artist = params; else if (label === "community playlist" || label === "playlist") map.playlist = params;
  });
...
function _plistRow(r) {
  ...
  if (!vid) return null;
  const { title: title, artist: artist, duration: duration } = _meta(_cols(r));
    ...
    for (const shelf of _items(home.contents)) {
      ...
      for (const node of _items(shelf.contents)) {
        ...
        out.push({
          ...
          thumbnail: node.thumbnail?.contents?.find(t => t?.url)?.url || node.thumbnails?.[0]?.url || ""
        });
...
async function like(session, videoId, like, client = "TV") {
  ...
  if (!r || r.status_code !== 200) {
    throw new Error("YouTube rejected the request. Your login may be invalid - use /account logout then /account login again.");
  }
}
...
async function radio(videoId, session = null) {
  ...
  const panel = r?.data?.contents?.singleColumnMusicWatchNextResultsRenderer?.tabbedRenderer?.watchNextTabbedResultsRenderer?.tabs?.[0]?.tabRenderer?.content?.musicQueueRenderer?.content?.playlistPanelRenderer;
  const items = [];
  const seen = new Set;
  for (const c of panel?.contents || []) {
    ...
    if (!title) continue;
    const artist = (v?.longBylineText?.runs || []).map(x => x.text || "").join("").split("•")[0].trim();
    ...
    items.push({
      ...
      thumbnail: v?.thumbnail?.thumbnails?.[(v?.thumbnail?.thumbnails?.length || 1) - 1]?.url || "",
...
export { sSongs, getRelated, getTrack, searchSuggestions, rawSearch, searchType, plist, liked, picks, libs, like, radio };
export default {
  sSongs: sSongs,
