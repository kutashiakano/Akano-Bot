...
async function aoyoAi(content) {
  async function getSearchResults(query) {
    ...
    const headers = { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json, text/plain, */*' };
  ...
  const hasilPencarian = searchResults.map(result => ({ judul: result.title, link: result.link, snippet: result.snippet }));
