...
const swapface = {
    create: async (target, source) => {
        ...
        try {
            const response = await axios.post('https://aifaceswapper.io/api/nicefish/fs/singleface', formData, config)
            ...
            while (status === 'waiting' && retries < 20) {
                const resultx = await axios.get(`https://aifaceswapper.io/api/nicefish/fs/result?request_id=${requestId}`, config)
...
function token() {
    ...
    const pea = [1, 2, 3].map(() => Math.random().toString(16).slice(2, 6)).join('-') + Math.random().toString(16).slice(2, 5)
    return `${ts}-${pea}`
}
