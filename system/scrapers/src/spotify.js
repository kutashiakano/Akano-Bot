...
class SpotifyScraper {
  ...
  async getSession(force = false) {
    if (!force && this.session && Date.now() - this.sessionAt < 5 * 60 * 1000) {
      return this.session;
    }
    ...
    this.session = { sessionCookie, dynamicName, dynamicValue };
  ...
  async _resolve(queryOrUrl) {
    ...
    const response = await axios.post(
      ...
      { headers: this._headers(sessionCookie), timeout: 60000 },
    );
    ...
    const cheerio = require("cheerio"); const $ = cheerio.load(response.data.data || "");
    const tracks = [];
    $('form[name="submitspurl"]').each((i, formElem) => {
      ...
      if (data && base && token) {
        tracks.push({ metadata: decodeMetadata(data), form: { data, base, token } });
      }
    });
  ...
  async search(query, limit = 10) {
    ...
    const promise = (async () => {
      try {
        const { tracks } = await this._resolve(normalized);
  ...
  async getTrack(url) {
    const { tracks } = await this._resolve(url);
    if (!tracks.length) {
  ...
  async getLinks(url) {
    const { tracks, sessionCookie } = await this._resolve(url);
    if (!tracks.length) {
      throw new Error("Track not found on Spotify");
    }
    const links = await this._getLinks(tracks[0].form, sessionCookie);
    return { metadata: this.toTrack(tracks[0].metadata, url), links };
  }

  async download(url, options = {}) {
    const outputDir =
      options.outputDir ||
      path.join(this.baseDir, `spdl_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);
