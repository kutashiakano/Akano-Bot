...
async function geniusSearch(query) {
  const res = await fetch(`https://genius.com/api/search/multi?per_page=5&q=${encodeURIComponent(query)}`);
  ...
  return hits.map(h => {
    ...
    return {
      ...
      artist: primary,
      url: r.url || "",
      thumbnail: (r.header_image_thumbnail_url || r.song_art_image_thumbnail_url || "").replace("1000x1000x1", "316x316x1"),
...
async function geniusLyrics(songUrl) {
  ...
  if (containers.length) {
    text = containers.map((i, el) => $(el).html()).get().join("\n");
    text = text.replace(/<br\s*\/?>/gi, "\n").replace(/\u00a0/g, " ");
  } else {
    text = stripHtml(res.body.match(/<div[^>]*class="[^"]*lyrics[^"]*"[^>]*>([\s\S]*?)<\/div>/i)?.[1] || "");
  }
  if (!text) return null;
...
function decodeEntities(text) {
  return String(text || "").replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&apos;/g, "'").replace(/&nbsp;/g, " ").replace(/&#(\d+);/g, (_, n) => String.fromCodePoint(parseInt(n, 10))).replace(/&#x([0-9a-f]+);/gi, (_, h) => String.fromCodePoint(parseInt(h, 16)));
}

async function lrclibLyrics(query, artist) {
  try {
    if (artist) {
      const res = await fetch(`https://lrclib.net/api/get?track_name=${encodeURIComponent(query)}&artist_name=${encodeURIComponent(artist)}`, {
    ...
    const res = await fetch(`https://lrclib.net/api/search?q=${encodeURIComponent(artist ? `${query} ${artist}` : query)}`, {
      Accept: "application/json"
    });
    if (res.status !== 200) return null;
...
export { getLyrics, geniusSearch, geniusLyrics, lrclibLyrics };
export default {
  getLyrics: getLyrics,
