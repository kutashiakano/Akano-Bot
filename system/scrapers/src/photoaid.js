...
async function photoaid(imageInput, type) {
    ...
    const endpoints = {
        enlarger: { upload: '/ai-image-enlarger/upload', result: '/ai-image-enlarger/result', referer: 'https://photoaid.com/en/tools/ai-image-enlarger' },
        relighting: { upload: '/relighting/upload', result: '/relighting/result', referer: 'https://photoaid.com/en/tools/remove-shadow' },
        removebg: { upload: '/remove-background/upload', result: '/remove-background/result', referer: 'https://photoaid.com/en/tools/remove-background' }
    };
    ...
    try {
        ...
        let final_result;
        do {
            final_result = await axios.post('https://photoaid.com/en/tools/api/tools/result', 
                { request_id: uploadResponse.data.request_id, reqURL: endpoints[type].result }, 
                {
                    headers: { 
                        ...headers, 
                        'referer': endpoints[type].referer,
