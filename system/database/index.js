...
for (const p of [ DATA_DIR, DASHBOARD_DIR ]) {
  try {
    fs.mkdirSync(p, {
      recursive: true
    });
...
function load() {
  ...
  try {
    if (!data.telegram || typeof data.telegram !== "object") data.telegram = {};
    if (!data.telegram.users || typeof data.telegram.users !== "object") data.telegram.users = {};
    if (!data.discord || typeof data.discord !== "object") data.discord = {};
    if (!data.discord.users || typeof data.discord.users !== "object") data.discord.users = {};
...
export { get, getRaw, write, update, withLock, ensureStructure, writeFileAtomic, DEFAULTS, TEXTS_DEFAULTS, ensureTexts, ensureWhatsApp, ensureTelegram, ensureDiscord, logError, query, scheduleAutosave, flush, flushSync, flushDatabaseAndExit };
export const defaultTexts = TEXTS_DEFAULTS;
export default {
  path: DB_FILE,
  file: DB_FILE,
  get: get,
