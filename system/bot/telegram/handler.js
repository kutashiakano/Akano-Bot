...
async function _getAvatarUrl(userId) {
  try {
    if (telegramBot && typeof telegramBot.getAvatarUrl === "function") return telegramBot.getAvatarUrl(userId);
  } catch {}
  return null;
}
  ...
  setup(bot) {
    bot.on("message", async ctx => {
      ...
      if (plugin && plugin.command) {
        ...
        if (!cmds.filter(c => typeof c === "string").map(c => c.toLowerCase()).includes(command)) plugin = null;
      }
      if (!plugin) {
    ...
    bot.on("callback_query:data", async ctx => {
      ...
      } else if (data && data.startsWith("dl:")) {
        const dlPlugin = global.telegramPlugins["dl"];
        if (dlPlugin && typeof dlPlugin.onCallback === "function") {
          try {
            await dlPlugin.onCallback(ctx);
          } catch (e) {
            global.logError("telegram.dlCallback", e);
          }
        }
