...
const Bot = tgsdk.Bot;
const session = tgsdk.session;
const hydrateFiles = tgsdk.hydrateFiles;

if (!Bot) logger.warn("grammy not installed, Telegram bot disabled");
if (!hydrateFiles) logger.warn("@grammyjs/files not installed, file download will use fallback URL");
...
class TelegramBot {
  ...
  async initialize(tokenOverride) {
    ...
    try {
      ...
      this.bot.catch(err => {
        const d = tgsdk.classifyError(err);
        if (tgsdk.isConflict(err)) {
...
instance.getAvatarUrl = getAvatarUrl;

export default instance;
export { TelegramBot, instance };
