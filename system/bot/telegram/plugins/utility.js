    ...
    if (text.startsWith("@")) {
      ...
      if (group && group.members) {
        for (const memberId in group.members) {
          ...
          if (m.username && m.username.toLowerCase() === targetUsername) {
...
module.exports = {
  group: true,
  tags: ['utility'],
  command: ['pin', 'unpin', 'lock', 'unlock', 'setrules', 'rules', 'report', 'admins', 'promote', 'demote'],
  ...
  run: async (ctx, args) => {
    ...
    if (command === "admins") {
      try {
        ...
        const list = chatAdministrators
          .filter((adm) => !adm.user.is_bot)
          .map((adm) => `• <b>${adm.user.first_name}</b> (@${adm.user.username || "no_username"}) [${adm.status}]`)
          .join("\n");
    ...
    switch (command) {
      ...
      case "promote":
        ...
        if (!targetPromo) return ctx.reply("Error: Please reply to a message or mention the user to promote.");
        ...
        ctx.reply(`[Promoted] <b>${targetPromo.firstName}</b> has been promoted to Administrator.`, { parse_mode: "HTML" });
        break;

      case "demote":
        const targetDemo = await getTargetUser(ctx, args);
        if (!targetDemo) return ctx.reply("Error: Please reply to a message or mention the user to demote.");
