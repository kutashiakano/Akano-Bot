...
module.exports = {
  ...
  tags: ['moderation'],
  command: ['mute', 'unmute', 'ban', 'unban', 'kick', 'warn', 'unwarn', 'warnings', 'purge', 'slowmode'],
  run: async (ctx, args) => {
    ...
    switch (command) {
      ...
      case "warn":
        ...
        if (warnings[target.id] >= 3) {
          ...
          ctx.reply(`[Banned] User ${target.firstName} reached 3 warnings and was banned from the group.`, { parse_mode: "HTML" });
        } else {
          ctx.reply(`[Warning] User ${target.firstName} has been warned. Total warnings: ${warnings[target.id]}/3.`, { parse_mode: "HTML" });
        }
        break;
      ...
      case "unwarn":
        ...
        if (dbUnwarn.groups[groupIdUnwarn].warnings && dbUnwarn.groups[groupIdUnwarn].warnings[target.id]) {
          ...
          ctx.reply(`[Warning] Removed 1 warning from ${target.firstName}. Total: ${dbUnwarn.groups[groupIdUnwarn].warnings[target.id]}/3.`, { parse_mode: "HTML" });
        } else {
      ...
      case "warnings":
        ...
        ctx.reply(`[Warning] User ${target.firstName} has ${count}/3 warnings.`, { parse_mode: "HTML" });
        break;
    }
  }
};
