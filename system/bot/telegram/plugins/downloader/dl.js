...
function formatKeyboard(hash, isAudio) {
  return tgsdk.keyboard([[
    isAudio ? [ "Video", `dl:${hash}:v` ] : [ "Audio", `dl:${hash}:a` ]
  ]]).reply_markup;
}
...
export default define({
  ...
  onCallback: async ctx => {
    ...
    if (!entry) {
      await tgsdk.answerTap(ctx, global.settings.message.expiredButton);
      return;
    }
    const isAudio = parts[2] === "a";
    await tgsdk.answerTap(ctx, isAudio ? global.settings.message.fetchingAudio : global.settings.message.fetchingVideo);
    try {
