...
async function runSticker(ctx, args) {
  ...
  await ctx.replyWithSticker(tgsdk.mediaFile(png, "sticker.png")).catch(async () => {
    await ctx.replyWithDocument(tgsdk.mediaFile(png, "sticker.png"), { caption: "Static sticker (PNG 512px)" }).catch(() => {});
  });
}
...
async function runKang(ctx, args) {
  ...
  if (!fileId) {
    ...
    if (!buf) return ctx.reply(global.settings.message.imageNeed).catch(() => {});
    const png = await toPng512(buf);
    const up = await ctx.api.uploadStickerFile(uid, "static", tgsdk.mediaFile(png, "kang.png"));
    fileId = up.file_id;
  }
...
async function runGetSticker(ctx) {
  ...
  try {
    await ctx.replyWithDocument(tgsdk.mediaFile(p), { caption: `${kind} sticker ${stk.emoji || ""} (${(stk.file_size / 1024).toFixed(0)}KB)` }).catch(() => {});
  } finally {
...
async function runGetPack(ctx, args) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try {
      ...
      try {
        await ctx.replyWithDocument(tgsdk.mediaFile(p, `${set.name}_${i}${ext}`)).catch(() => {});
      } finally {
        try { await fs.promises.unlink(p).catch(() => {}); } catch {}
      }
    } catch {}
...
async function runConvert(ctx, toMp4) {
  ...
  try {
    ...
    if (toMp4) await ctx.replyWithVideo(tgsdk.mediaFile(out)).catch(() => {});
    else await ctx.replyWithDocument(tgsdk.mediaFile(out, "converted.webm")).catch(() => {});
  } finally {
