...
let _unified = null;
try {
  const m = await import("../../../../scrapers/src/unified-downloader.js");
  _unified = m.default ?? m;
} catch {}
...
async function downloadMedia(ctx, url, isAudio = false) {
  try {
    ...
    if (source === "spotify" && global.scraper?.spotify) {
      ...
      await ctx.reply(global.settings.message.fetchingSpotify);
      ...
      await tgsdk.sendRetry(() => ctx.replyWithAudio(toInputFile(file), {
    ...
    await ctx.reply(global.settings.message.downloadingMedia);

    const cookiesPath = path.join(import.meta.dirname, "../../../../../cookies.txt");
    ...
    if (isGallery && validFiles.length > 1) {
      ...
      if (mediaGroup.length > 0) {
        await tgsdk.sendRetry(() => ctx.replyWithMediaGroup(mediaGroup)).catch(async () => {
          for (const file of validFiles) {
            const fi = getFileInfo(file);
            const input = toInputFile(file);
            if (fi.type === "video") {
              await tgsdk.sendRetry(() => ctx.replyWithVideo(input, {
                caption: caption,
                ...HTML
              })).catch(() => {});
            } else if (fi.type === "audio") {
              await tgsdk.sendRetry(() => ctx.replyWithAudio(input, {
                caption: caption,
                ...HTML
              })).catch(() => {});
            } else if (fi.type === "photo") {
              await tgsdk.sendRetry(() => ctx.replyWithPhoto(input, {
                caption: caption,
                ...HTML
              })).catch(() => {});
            } else {
              await tgsdk.sendRetry(() => ctx.replyWithDocument(input, {
                caption: caption,
                ...HTML
              })).catch(() => {});
            }
      ...
      if (isAudioBool || fileInfo.type === "audio") {
        ...
        await tgsdk.sendRetry(() => ctx.replyWithVideo(toInputFile(file), {
          caption: caption,
          ...HTML
        }));
      } else if (fileInfo.type === "photo") {
        await tgsdk.sendRetry(() => ctx.replyWithPhoto(toInputFile(file), {
          caption: caption,
          ...HTML
        }));
      } else {
        await tgsdk.sendRetry(() => ctx.replyWithDocument(toInputFile(file), {
          caption: caption,
          ...HTML
        }));
      }
...
export { getFileInfo, buildCaption, detectSource, downloadMedia };
export default {
  getFileInfo,
  buildCaption,
  detectSource,
