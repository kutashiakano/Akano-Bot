...
function ageKeyboard() {
  const ages = Array.from({ length: 22 }, (_, i) => 30 - i);
  const rows = [];
  for (let i = 0; i < ages.length; i += 6) {
    rows.push(
      ages.slice(i, i + 6).map((a) => ({
        text: `${a} Years`,
        callback_data: `age:${a}`,
      })),
    );
  ...
  rows.push([
    { text: "Random Years", callback_data: "age:random" },
    { text: "Cancel", callback_data: "reglang:cancel" },
  ]);
...
module.exports = {
  ...
  onCallback: async (ctx) => {
    ...
    if (data.startsWith("age:")) {
      ...
      if (isNaN(age) || age < 5 || age > 30) {
        await ctx.answerCbQuery("Age must be between 5 and 30.").catch(() => {});
        return;
      }
      setProfile(userId, { age, registered: true, regTime: Date.now() });
      await ctx
        .editMessageText(
  ...
  run: async (ctx) => {
    ...
    if (getProfile(userId)?.registered) {
      return ctx
        .reply("You are already registered.\nUse /unreg to reset your registration.")
        .catch(() => {});
    }
    const text = "Select Your Age";
    await ctx
      .replyWithPhoto(global.settings.cover, { caption: text, reply_markup: ageKeyboard() })
      .catch(async () => {
        await ctx.reply(text, { reply_markup: ageKeyboard() }).catch(() => {});
      });
  },
};
