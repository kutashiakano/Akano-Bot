  ...
  run: async ctx => {
    ...
    switch (command) {
     ...
     case "warn":
      ...
      if (!db.telegram.groups[groupId]) db.telegram.groups[groupId] = { warnings: {} };
     ...
     case "unwarn":
      ...
      const unwarnGroup = dbUnwarn.telegram?.groups?.[groupIdUnwarn];
      const unwarnWarns = unwarnGroup?.warnings;
      if (unwarnWarns && unwarnWarns[target.id]) {
        unwarnWarns[target.id] = Math.max(0, unwarnWarns[target.id] - 1);
        database.write(dbUnwarn);
        ctx.reply(`[Warning] Removed 1 warning from ${target.firstName}. Total: ${unwarnWarns[target.id]}/3.`, HTML);
      } else {
     ...
     case "warnings":
      ...
      const count = dbWarns.telegram?.groups?.[groupIdWarns]?.warnings?.[target.id] || 0;
