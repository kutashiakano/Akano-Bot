import "../../../settings.js";
import { smsg } from "./lib/index.js";
import util from "util";
import moment from "moment-timezone";
import fs from "fs";
import path from "path";
import { Cooldown, SpamDetection, matcher, Queue, isVirtex, extractLinks, whatsapp, verify } from "@kutashiakanocanzy/sdk";
import { ensureWhatsApp } from "../../database/index.js";
...
function getVerifier() {
  ...
  try {
    if (global.db?.data && typeof global.db.data === "object" && !global.db.data.verifications) global.db.data.verifications = {};
  } catch {}
  return verify.createVerifier({
    state: global.db?.data?.verifications || {},
    chats: jid => global.db?.data?.chats?.[jid] || {},
    save: async () => {
      try {
    ...
    logError: (tag, e) => {
      try {
        global.logError(tag, e);
      } catch {}
    },
    fetchGroupMetadata: async (gid, s) => await global.store?.fetchGroupMetadata(gid, s),
    config: {
      enabled: cfg.enabled !== false,
      captchaLength: Number(cfg.captchaLength) > 0 ? Number(cfg.captchaLength) : 6,
      expiresIn: Number(cfg.expiresIn) > 0 ? Number(cfg.expiresIn) : 3e5,
      maxAttempts: Number(cfg.maxAttempts) > 0 ? Number(cfg.maxAttempts) : 3
    }
  });
}

const getCooldown = () => global.settings?.system?.cooldown || 5e3;

const getSpamConfig = () => ({
  RESET_TIMER: global.settings?.system?.spam?.resetTimer || 5e3,
  HOLD_THRESHOLD: global.settings?.system?.spam?.holdThreshold || 5,
  PERM_T: global.settings?.system?.spam?.permanentThreshold || 10,
  BAN_T: global.settings?.system?.spam?.bannedThreshold || 15,
  NOT_T: global.settings?.system?.spam?.notifyThreshold || 3
});
...
const pickRandom = arr => Array.isArray(arr) && arr.length ? arr[Math.floor(Math.random() * arr.length)] : "";
...
async function getCachedGroupMetadata(sock, chat) {
  ...
  const metadata = await global.store.fetchGroupMetadata(chat, sock);
...
async function handler(chatUpdate) {
    ...
    if (!m) return;
    try {
      m = await smsg(this, m, { store: global.store }) || m;
      if (!m) return;
      ...
      if (m.isGroup && typeof m.sender === "string") {
        try {
          ...
          if (m.sender.endsWith("lid")) {
            const found = groupMeta.participants?.find(v => typeof v.lid === "string" && v.lid === m.sender || typeof v.id === "string" && v.id === m.sender);
          ...
          if (m.quoted && typeof m.quoted.sender === "string" && m.quoted.sender.endsWith("lid")) {
            const found = groupMeta.participants?.find(v => typeof v.lid === "string" && v.lid === m.quoted.sender || typeof v.id === "string" && v.id === m.quoted.sender);
            if (found && typeof found.phoneNumber === "string") m.quoted.sender = found.phoneNumber;
          }
        } catch (e) {}
      }
      const isNewsletter = isNews(m.sender);
      if (isNewsletter) return;
      try {
        if (!m.fromMe && m.quoted && typeof m.quoted.id === "string" && await getVerifier().handleReply(this, m)) return;
      } catch (e) {
        global.logError("whatsapp.verification.reply", e);
      }
      try {
        ensureWhatsApp(global.db.data, m);
      } catch (e) {}
      if (typeof m.sender !== "string") return;
      if (!m.sender.includes("@")) return;
      if (typeof m.chat !== "string") return;
      const isROwner = [ sock.decodeJid(global.sock.user.id), ...global.owner.map(a => a + "@s.whatsapp.net") ].includes(m.sender);
      ...
      if (m.isGroup) {
        ...
        let member = (await store.fetchGroupMetadata(m.chat, this)).participants.map(a => a.id);
      ...
      if (m.text && !isOwner && !isPrems && !isMods) {
        ...
        if (spamResult.state === "BANNED" || spamResult.state === "PERMANENT") {
      ...
      if (m.isGroup) {
        ...
        const sndPart = _participants.find(u => sock.decodeJid(u.id) === m.sender) || {};
        const _botParticipant = _participants.find(u => sock.decodeJid(u.id) == sock.decodeJid(this.user.id)) || {};
        const senderIsAdmin = sndPart.admin === "admin" || sndPart.admin === "superadmin";
        ...
        if ((chat.antilink || antilinkGlobal) && !senderIsAdmin && !isOwner && m.text) {
          ...
          if (links.length > 0) {
            ...
            m.reply(global.settings?.message?.antilink || `@${senderNum} links are not allowed in this group!`, null, {
              mentions: [ m.sender ]
            });
            return;
          }
        ...
        if ((chat.antivirtex || antivirtexGlobal) && !senderIsAdmin && !isOwner && m.text) {
          if (isVirtex(m.text)) {
            ...
            m.reply(global.settings?.message?.antivirtex || `@${senderNum} long text is not allowed in this group!`, null, {
              mentions: [ m.sender ]
            });
            return;
          }
      ...
      const useQueue = (global.settings.opts?.queque || global.settings.opts?.queue || global.settings.opts?.pending) && global.msgqueue;
      let _queueId = null;
      if (useQueue && m.text && !m.fromMe && !(isMods || isPrems || isOwner)) {
        _queueId = m.id || m.key?.id;
        if (_queueId) {
          global.msgqueue.add(_queueId);
          try {
            await global.msgqueue.waitQueue(_queueId, 6e4);
          } catch (e) {
            if (String(e.message).includes("timeout")) {
              global.msgqueue.delete(_queueId);
            } else {}
          }
        }
      }
      this.msgqueque = this.msgqueque || [];
      if (_queueId) this.msgqueque.push(_queueId);
      ...
      if (!m.fromMe && !isOwner && !isPrems && !isMods && opts["selfMode"]) return;
      ...
      if (setting.groupmode && !m.isGroup && !isPrems && !isOwner && !m.fromMe) {
        m.reply(` Using bot in private chat only for premium user, want to upgrade to premium plan? send *.premium* to see benefit and prices.`);
        return;
      }
      if (typeof m.text !== "string") m.text = "";
      try {
        const tapId = whatsapp.tap(m);
        if (tapId) m.text = tapId;
      } catch {}
      if (m.isBaileys) return;
      if (!m.isGroup && !m.fromMe && !/buttonsResponseMessage|listResponseMessage|templateButtonReplyMessage|interactiveResponseMessage/.test(m.mtype || "")) {
        try {
          const registerPlugin = (await import("./plugins/settings/register.js?t=" + Date.now()).then(m => m.default || m).catch(() => null));
          if (registerPlugin && !global.db?.data?.users?.[m.sender]?.registered && !isOwner) {
            await registerPlugin.sndAgeSel(this, m.chat, {
              quoted: m,
              usedPrefix: ".",
              name: (m.pushName || "User").trim()
            });
          }
        } catch (e) {}
      }
      m.exp += Math.ceil(Math.random() * 1e3);
      let usedPrefix;
      ...
      for (let name in global.plugin) {
        ...
        let _prefix = plugins.customPrefix ? plugins.customPrefix : sock.prefix ? sock.prefix : global.prefix;
        ...
        if (opts && match && m) {
          ...
          usedPrefix = result;
          let noPrefix = isOwner ? !result ? m.text : m.text.replace(result, "") : !result ? "" : m.text.replace(result, "").trim();
          ...
          const prefixCommand = !result ? plugins.customPrefix || plugins.command : plugins.command;
          ...
          if (m.isCmd && !isOwner && !isPrems && !isMods) {
            if (cooldown.has(m.sender, command)) {
              ...
              m.reply(global.settings?.message?.cooldown || `Cooldown! Wait ${Math.ceil(remaining / 1e3)}s`);
              continue;
            }
          ...
          if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
            ...
            if (name != "owner-unbanchat.js" && chat && chat.isBanned && !isOwner) return;
            if (name != "group-unmute.js" && chat && chat.mute && !isAdmin && !isOwner) return;
          }
          ...
          if (plugins.reg && !users?.registered && !isOwner && !isROwner) {
            m.reply(settings.message.needRegister.replace("{prefix}", usedPrefix));
            try {
              const registerPlugin = (await import("./plugins/settings/register.js?t=" + Date.now()).then(m => m.default || m).catch(() => null));
              if (registerPlugin) {
                await registerPlugin.sndAgeSel(this, m.chat, {
          ...
          if (plugins.limit && users && typeof users.limit === "number" && users.limit < 1 && !isPrems && !isOwner) {
            m.reply(settings.message.limitReached);
            continue;
          }
          m.isCmd = true;
          m.cmd = command;
          ...
          m.exp += xp;
          if (plugins.limit && users && typeof users.limit === "number" && users.limit > 0 && !isPrems && !isOwner) {
            const cost = plugins.limit.constructor.name === "Boolean" ? 1 : plugins.limit;
            if (users.limit >= cost) {
              users.limit -= cost;
            } else {
              m.reply(settings.message.limitNotEnough);
              continue;
            }
          }
          let extra = {
      ...
      if (!isCommand && m.text) {
        ...
        if (body && !isNaN(body) && global.typo.has(senderKey)) {
          ...
          if (session.commands && session.commands[choice]) {
            ...
            body = prefix + selectedCommand + (text ? " " + text : "");
            if (session.quoted) {
              m.quoted = session.quoted;
            }
            global.typo.delete(senderKey);
            await m.reply(` Executing *${prefix + selectedCommand}*...`);
            isCommand = true;
          }
        }
      }
      if (!isCommand && m.text && Array.isArray(global.prefix) && global.prefix.some(p => m.text.startsWith(p))) {
        ...
        for (const name in global.plugin) {
          ...
          if (pl?.command) {
            if (Array.isArray(pl.command)) allCmds.push(...pl.command); else allCmds.push(pl.command);
          }
          if (pl?.help) {
            if (Array.isArray(pl.help)) allCmds.push(...pl.help); else allCmds.push(pl.help);
          }
        }
      ...
      try {
        const qId = typeof _queueId !== "undefined" && _queueId ? _queueId : m?.id || m?.key?.id;
        if (qId) {
          if (global.msgqueue) {
            try {
              global.msgqueue.unqueue(qId);
            } catch {}
            if (!global.msgqueue.isEmpty()) {
              const nxt = global.msgqueue.first();
              if (nxt) try {
                global.msgqueue.emit(nxt);
              } catch {}
            }
          }
          if (this.msgqueque) {
            const idx = this.msgqueque.indexOf(qId);
            if (idx !== -1) this.msgqueque.splice(idx, 1);
          }
        } else if (opts["queque"] && m?.text) {
...
async function participantsUpdate({ id: id, author: author, participants: participants, action: action }) {
    const sock = this;
    const store = global.store;
    const opts = global.opts || {};
    const db = global.db;
    ...
    try {
      ...
      const fallbackPp = path.join(import.meta.dirname, "..", "..", "..", "image", "akano.jpg");
      const toJid = p => {
      ...
      switch (action) {
       ...
       case "remove":
        if (chat.welcome || action === "remove" && chat.left) {
          for (let raw of participants) {
            ...
            const reasn = check ? `${action === "add" ? "added" : "removed"} by @${author.split("@")[0]}` : `${action === "add" ? "Joined the group" : "Left the group"}`;
            let pp = "";
            try {
              pp = await this.profilePictureUrl(user, "image");
            } catch (e) {}
            const tmpl = action === "add" ? chat.sWelcome || chat.welcomeMsg || pickRandom(dbTexts().welcomeMsgs) || "Welcome @user to @subject!\nDon't forget to read the rules!" : chat.sBye || chat.leaveMsg || pickRandom(dbTexts().goodbyeMsgs) || "Goodbye @user!\nWe'll miss you!";
            const text = tmpl.replace(/@user/g, "@" + user.split("@")[0]).replace(/@subject/g, groupName).replace(/@desc/g, groupMetadata.desc?.toString() || "").replace(/%member/g, "@" + user.split("@")[0]).replace(/%subject/g, groupName).replace(/%reason/g, reasn).replace(/%time/g, moment.tz("Asia/Jakarta").format("HH:mm"));
            let thumb = null;
            if (pp) {
              try {
                thumb = await sock.resize(pp, 300, 200);
              } catch (e) {}
            }
            if (!thumb) {
              try {
                thumb = fs.readFileSync(fallbackPp);
              } catch (e) {}
            }
       ...
       case "demote":
        ...
        if (chat.detect) {
          const tmpl = action === "promote" ? chat.sPromote || pickRandom(dbTexts().promoteMsgs) || "@user is now an admin!" : chat.sDemote || pickRandom(dbTexts().demoteMsgs) || "@user is no longer an admin.";
...
async function joinRequestUpdate(update) {
    const opts = global.opts || {};
    if (global.isInit) return;
    if (opts["selfMode"]) return;
    try {
      await getVerifier().onJoinRequest(this, update);
    } catch (e) {
      global.logError("whatsapp.joinRequestUpdate", e);
    }
  }
export { handler, participantsUpdate, joinRequestUpdate };
export default { handler, participantsUpdate, joinRequestUpdate };