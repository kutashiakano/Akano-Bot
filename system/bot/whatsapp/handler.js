    ...
    try {
        ...
        try {
          ...
          if (m.sender.endsWith("lid")) {
            const found = groupMeta.participants?.find(
              (v) =>
                (typeof v.lid === "string" && v.lid === m.sender) ||
                (typeof v.id === "string" && v.id === m.sender),
            );
          ...
          if (m.quoted && typeof m.quoted.sender === "string" && m.quoted.sender.endsWith("lid")) {
            const found = groupMeta.participants?.find(
              (v) =>
                (typeof v.lid === "string" && v.lid === m.quoted.sender) ||
                (typeof v.id === "string" && v.id === m.quoted.sender),
            );
      ...
      try {
        if (
          !m.fromMe &&
          m.quoted &&
          typeof m.quoted.id === "string" &&
          (await require("./lib/verification").handleReply(this, m))
        )
          return;
      } catch (e) {
      ...
      if (m.isGroup) {
        ...
        const sndPart =
          _participants.find((u) => sock.decodeJid(u.id) === m.sender) || {};
        const _botParticipant =
          _participants.find((u) => sock.decodeJid(u.id) == sock.decodeJid(this.user.id)) || {};
        const senderIsAdmin =
          sndPart.admin === "admin" || sndPart.admin === "superadmin";
        const botIsAdmin =
          _botParticipant.admin === "admin" || _botParticipant.admin === "superadmin";
        ...
        if ((chat.antilink || antilinkGlobal) && !senderIsAdmin && !isOwner && m.text) {
          ...
          if (links.length > 0) {
            ...
            m.reply(
              global.settings?.message?.antilink ||
        ...
        if ((chat.antivirtex || antivirtexGlobal) && !senderIsAdmin && !isOwner && m.text) {
          if (isVirtex(m.text)) {
            ...
            m.reply(
              global.settings?.message?.antivirtex ||
                `@${senderNum} long text is not allowed in this group!`,
              null,
              { mentions: [m.sender] },
            );
            return;
      ...
      if (
        (global.settings.opts.queque || global.settings.opts.pending) &&
        m.text &&
        !(isMods || isPrems)
      ) {
      ...
      if (opts["autoRead"] || global.settings?.automation?.autoRead)
      ...
      if (setting.groupmode && !m.isGroup && !isPrems && !isOwner && !m.fromMe) {
        m.reply(
          ` Using bot in private chat only for premium user, want to upgrade to premium plan? send *.premium* to see benefit and prices.`,
        );
        return;
      }
      ...
      if (
        !m.isGroup &&
        !m.fromMe &&
        !/buttonsResponseMessage|listResponseMessage|templateButtonReplyMessage|interactiveResponseMessage/.test(
          m.mtype || "",
        )
      ) {
        try {
        ...
        if (opts && match && m) {
          ...
          if (m.isCmd && !isOwner && !isPrems && !isMods) {
            if (cooldown.has(m.sender, command)) {
              ...
              m.reply(
                global.settings?.message?.cooldown ||
                  `Cooldown! Wait ${Math.ceil(remaining / 1000)}s`,
              );
              continue;
            }
          ...
          if (opts["groupOnly"] && !m.fromMe && !m.chat.endsWith("g.us")) {
            let contactOwner = global.owner
              .map((a, i) => `*• Contact ${i + 1} :* wa.me/` + a)
              .join("\n");
      ...
      if (
        ...
        if (inputCmd) {
          ...
          if (suggestions.length > 0) {
            ...
            caption += suggestions
              .map((v, i) => `*${i + 1}.* ${usedP}${v.string} (${v.accuracy}%)`)
              .join("\n");
      ...
      try {
        require("../print")({ type: "whatsapp", m, sock: this });
      } catch (e) {}
            ...
            for (let raw of participants) {
              ...
              const tmpl =
                action === "add"
                  ? chat.sWelcome ||
                    chat.welcomeMsg ||
                    pickRandom(dbTexts().welcomeMsgs) ||
                    "Welcome @user to @subject!\nDon't forget to read the rules!"
                  : chat.sBye ||
                    chat.leaveMsg ||
                    pickRandom(dbTexts().goodbyeMsgs) ||
                    "Goodbye @user!\nWe'll miss you!";
                ...
                .sendIAMessage(
                  ...
                  null,
                  {
                    ...
                    media: {
                      ...
                      name:
                        action === "add"
                          ? pickRandom(dbTexts().welcomeNames) || "Welcome"
                          : pickRandom(dbTexts().goodbyeNames) || "Goodbye",
                      address:
                        action === "add"
                          ? pickRandom(dbTexts().welcomeAddrs)
                          : pickRandom(dbTexts().goodbyeAddrs),
                    },
                    mentions: [user],
                  },
                )
          ...
          if (chat.detect) {
            const tmpl =
              action === "promote"
                ...
                : chat.sDemote ||
                  pickRandom(dbTexts().demoteMsgs) ||
                  "@user is no longer an admin.";
            const text = tmpl.replace(/@user/g, "@" + target.split("@")[0]);
            await sock.sendMessage(id, { text, mentions: [target] }).catch(() => {});
          }
          break;
      }
    } catch (e) {
