...
const { Cooldown: Cooldown, SpamDetection: SpamDetection, matcher: matcher } = require("./lib/cooldown");
const { isVirtex: isVirtex, extractLinks: extractLinks } = require("./lib/anti");
...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
      ...
      try {
        if (!m.fromMe && m.quoted && typeof m.quoted.id === "string" && await require("./lib/verification").handleReply(this, m)) return;
      } catch (e) {
      ...
      if (!m.isGroup && !m.fromMe && !/buttonsResponseMessage|listResponseMessage|templateButtonReplyMessage|interactiveResponseMessage/.test(m.mtype || "")) {
        try {
          const registerPlugin = require("./plugins/settings/register");
  ...
  async participantsUpdate({ id: id, author: author, participants: participants, action: action }) {
  ...
  async joinRequestUpdate(update) {
    if (global.isInit) return;
    if (opts["selfMode"]) return;
    try {
      await require("./lib/verification").onJoinRequest(this, update);
    } catch (e) {
