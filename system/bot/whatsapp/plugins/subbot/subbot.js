...
function createSubSocket(connectionOptions) {
  return enhanceSocket(makeRawSocket(connectionOptions));
}
...
async function initializeSession(sessionPath) {
  ...
  let sock = createSubSocket(connectionOptions);
  sock.isInit = false;
  let isInit = true;
  ...
  const handler = {
    async setupEvents(restartConn) {
      if (restartConn) {
        ...
        sock = createSubSocket(connectionOptions);
        isInit = true;
      }
      if (!isInit) {
      ...
      const handlerMod = await loadHandlerModule();
