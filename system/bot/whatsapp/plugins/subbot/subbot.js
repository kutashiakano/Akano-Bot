const { useMultiFileAuthState: useMultiFileAuthState, DisconnectReason: DisconnectReason, fetchLatestBaileysVersion: fetchLatestBaileysVersion, makeCacheableSignalKeyStore: makeCacheableSignalKeyStore, Browsers: Browsers } = require("baileys");
const NodeCache = require("node-cache");
const crypto = require("crypto");
const fs = require("fs");
const path = require("path");
const pino = require("pino");
const { makeWASocket: makeWASocket } = require("../../lib");
...
try {
  const subCfg = (global.settings && (global.settings.subbot || (global.settings.whatsapp && global.settings.whatsapp.subbot))) || {};
  if (subCfg.AutoConnect && typeof autoConnectSessions === "function") {
    autoConnectSessions();
  }
} catch {}

async function autoConnectSessions() {
  const sessionDir = `./${(global.settings.subbot || global.settings.whatsapp?.subbot)?.sessionbot || "system/jadibot"}`;
  if (!fs.existsSync(sessionDir)) return;
  const sessionFolders = fs.readdirSync(sessionDir).filter(f => fs.statSync(path.join(sessionDir, f)).isDirectory());
...
async function initializeSession(sessionPath) {
  const { state: state, saveCreds: saveCreds } = await useMultiFileAuthState(sessionPath);
  if (!state.creds.registered) return;
  const msgRetryCounterCache = new NodeCache;
  const { version: version } = await fetchLatestBaileysVersion();
...
const __orig = {
  ...
  tags: [ "subbot" ],
  private: true,
  run: async (m, { sock: _sock, usedPrefix: usedPrefix, command: command, args: args }) => {
    const __sock = await global.sock;
    if (!(args[0] === "plz" || __sock.user.id === _sock.user.id)) {
      return m.reply(`This command can only be used on the main bot! wa.me/${__sock.user.id.split("@")[0]}?text=${usedPrefix}${command}`);
    }
    const handler = {
      async initializeBot() {
        let authFolderB = crypto.randomBytes(10).toString("hex").slice(0, 8);
        const sessionPath = `./${(global.settings.subbot || global.settings.whatsapp?.subbot)?.sessionbot || "system/jadibot"}/${authFolderB}`;
        ...
        if (!sock.authState.creds.registered) {
          ...
          setTimeout(async () => {
            ...
            const codeBot = await sock.requestPairingCode(cleanedNumber, randomPairing);
