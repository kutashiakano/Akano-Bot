let handler = async (m, {sock: sock, args: args, usedPrefix: usedPrefix, command: command, isOwner: _isOwner}) => {
  ...
  if (!event) {
    return m.reply([ `*SIMULATE* — Test welcome/leave/promote/demote`, "", `*Usage:*`, `${usedPrefix}simulate welcome @user`, `${usedPrefix}simulate bye @user`, `${usedPrefix}simulate promote @user`, `${usedPrefix}simulate demote @user`, "", `*Events:*`, `welcome/add/invite — simulate user join`, `bye/kick/leave/remove — simulate user leave`, `promote — simulate user promoted`, `demote — simulate user demoted` ].join("\n"));
  }
