let handler = async (m, { sock, args, usedPrefix }) => {
  ...
  if (!action || !["add", "remove", "list"].includes(action)) {
    const rows = [
      { title: "List Premium", description: "Show all premium users", id: `${usedPrefix}premium list` },
      { title: "Add Premium", description: "Add user to premium", id: `${usedPrefix}premium add` },
      { title: "Remove Premium", description: "Remove user from premium", id: `${usedPrefix}premium remove` },
    ];

    return sock.sendMessage(
      m.chat,
      {
        location: { name: "PREMIUM", address: "User Management", jpegThumbnail: null },
        ...
        nativeFlow: [
          {
            ...
            buttonParamsJson: JSON.stringify({
              title: "Select Action",
              sections: [{ title: "Premium", highlight_label: "Management", rows }],
            }),
          },
        ],
      },
  ...
  if (action === "list") {
    ...
    return m.reply(caption, null, { mentions: premiums.map(u => u.jid) });
  }
