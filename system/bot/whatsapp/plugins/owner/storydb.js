const __orig = {
  ...
  run: async (m, {sock: sock, text: text, command: command}) => {
    ...
    switch (command) {
     case "listsw":
      const stories = (await sock.story().catch(() => [])).sort((a, b) => a.name.localeCompare(b.name) || (a.type || "zzz").localeCompare(b.type || "zzz"));
     ...
     case "getstory":
      ...
      if (isNaN(index) || index < 0 || index >= data.length) return m.reply(`Invalid number (1-${data.length})`, m);
      ...
      sock.sendMessage(m.chat, payload, {
        quoted: m
      });
      break;
    }
