...
const PATTERNS = [ {
  id: "native-binding",
  test: e => /Cannot find native binding/i.test(e.message) || /@snazzah\/davey/i.test(e.origin + (e.stack || []).join(" ")),
  cause: "Native dependency (npm optional dependency) failed to install or is corrupted, usually because package-lock.json and node_modules are out of sync.",
  fix: [ "rm -rf node_modules package-lock.json", "npm i", "If it still fails: npm i @discordjs/voice --force" ]
}, {
  id: "enoent-rename",
  test: e => /ENOENT.*rename/i.test(e.message) || /rename.*database\.json/i.test(e.message),
  cause: "Multiple bot processes are writing to the same database, the .tmp file was removed before the rename finished.",
  fix: [ "Kill all old bot processes: pkill -f 'node index.js'", "Make sure there is no duplicate process before restarting" ]
}, {
  id: "max-listeners",
  test: e => /MaxListenersExceeded|listeners added/i.test(e.message),
  cause: "Handler reload keeps adding uncaughtException/unhandledRejection listeners without removing them.",
  fix: [ "Reduce handler reload frequency (WhatsApp lib/index.js)", "Safe to ignore temporarily, but consider adding process.setMaxListeners(20)" ]
}, {
  id: "timeout",
  test: e => /ETIMEDOUT|timeout|timed out/i.test(e.message),
  cause: "Network connection to a third-party server (TikTok/YT/etc.) is slow, blocked, or rate limited.",
  fix: [ "Check internet connection and VPN/proxy", "Retry after a moment" ]
}, {
  id: "instagram-login",
  test: e => /instagram.*login|login.*instagram/i.test(e.message),
  cause: "Instagram requires login (session expired / rate limited), scraping without a valid session.",
  ...
  cause: "Bot cannot join the voice channel: missing Connect/Speak permission or a broken existing connection.",
  fix: [ "Check bot permissions: Connect & Speak in the voice channel", "Move the bot out of the voice channel and try again" ]
}, {
  id: "discord-voice-binding",
  test: e => /@discordjs\/voice|@discordjs\/opus|play\/dca|prism-media/i.test(e.origin + (e.stack || []).join(" ")),
  cause: "Discord voice library is broken (native binding / codec).",
  fix: [ "rm -rf node_modules && npm i", "Make sure @discordjs/voice and @discordjs/opus are installed" ]
}, {
  id: "rate-limit",
...
function formatTime(entry) {
  return entry.wib || new Date(entry.ts).toLocaleString("en-US", {
...
let handler = async (m, {sock: sock, args: args}) => {
  ...
  if (cmd === "stats") {
    ...
    const topName = Object.entries(byName).sort((a, b) => b[1] - a[1]).slice(0, 5);
    const lines = [ "*ERROR LOG STATISTICS*", `Total errors (last 50): *${logs.length}*`, "", "*By Source:*", ...topSrc.map(([k, v]) => `> ${k}: ${v}x`), "", "*By Error Type:*", ...topName.map(([k, v]) => `> ${k}: ${v}x`) ];
    return m.reply(lines.join("\n"));
  }
  const num = parseInt(cmd, 10) || parseInt(args[0], 10);
  if (num && !isNaN(num)) {
    const entry = logs.find(l => l.id === num) || logs[num - 1];
    if (!entry) return m.reply(`Error with ID ${num} was not found. Use *.logerror* to see the list.`);
    const insight = analyze(entry);
    const stack = (entry.stack || []).slice(0, 10);
    const lines = [ `*ERROR DETAIL #${entry.id}*`, `Time   : ${formatTime(entry)}`, `Source : ${entry.source} | Type: ${entry.name}${entry.code ? ` | Code: ${entry.code}` : ""}`, "", `*Message:*`, truncate(entry.message, 400), `*Location:* ${entry.origin}`, "" ];
    if (stack.length) {
  ...
  if (!logs.length) return m.reply("No errors recorded yet. Log is stored at *system/database/logerror.json*.");
  const lines = [ `*ERROR LOG - LAST 5*`, `Total recorded: ${logs.length} errors`, "" ];
  logs.slice(-5).reverse().forEach((l, i) => {
