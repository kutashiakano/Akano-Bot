...
let handler = async (m, { sock, command, usedPrefix }) => {
  ...
  if (/^to(vn|ptt|voicenote)$/i.test(command)) {
    ...
    await sock.sendFile(m.chat, audio.data, "audio.ogg", "", m, true, { mimetype: "audio/ogg; codecs=opus" });
  }
  ...
  if (/^to(mp3|a(udio)?)$/i.test(command)) {
    ...
    if (!audio) return m.reply("Can't convert media to audio");
    await sock.sendFile(m.chat, audio.data, "audio.opus", "", m, null, { mimetype: "audio/ogg; codecs=opus" });
  }
};
handler.help = ["toimg", "tovideo", "tovn", "tovoicenote", "tomp3"];
handler.tags = ["tools"];
handler.command = /^(toimg|tovideo|to(vn|ptt|voicenote)|to(mp3|a(udio)?))$/;
