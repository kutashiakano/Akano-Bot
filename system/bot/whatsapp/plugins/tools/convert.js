const { ffmpeg, toPTT, toAudio } = require("../../lib/converter.js");
...
let handler = async (m, { sock, command, usedPrefix }) => {
  if (command === "toimg") {
    ...
    if (!/sticker/.test(mime))
      return m.reply(`Reply sticker with command *${usedPrefix + command}*`);
  ...
  if (command === "tovideo") {
    ...
    if (!/webp|audio/.test(mime))
      return m.reply(`Reply sticker/audio with command *${usedPrefix + command}*`);
    let media = await m.quoted.download();
    let out = null;
    if (/webp/.test(mime)) {
      out = await ffmpeg(
  ...
  if (/^to(vn|ptt|voicenote)$/i.test(command)) {
    ...
    if (!/video|audio/.test(mime))
      return m.reply(`Reply video/audio to convert to voice note with *${usedPrefix + command}*`);
  ...
  if (/^to(mp3|a(udio)?)$/i.test(command)) {
    ...
    let mime = (m.quoted || m.msg).mimetype || "";
    if (!/video|audio/.test(mime))
      return m.reply(`Reply video/voice note to convert to mp3 with *${usedPrefix + command}*`);
    let media = await q.download();
    if (!media) return m.reply("Can't download media");
