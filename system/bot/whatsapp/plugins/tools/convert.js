...
let handler = async (m, { sock: sock, command: command, usedPrefix: usedPrefix }) => {
  if (command === "toimg") {
    ...
    if (!/sticker/.test(mime)) return m.reply(global.settings.message.stickerReply.replace("{prefix}", usedPrefix).replace("{command}", command));
  ...
  if (command === "tovideo") {
    if (!m.quoted) return m.reply(global.settings.message.stickerAudioReply.replace("{prefix}", usedPrefix).replace("{command}", command));
    let mime = m.quoted.mimetype || "";
    if (!/webp|audio/.test(mime)) return m.reply(global.settings.message.stickerAudioReply.replace("{prefix}", usedPrefix).replace("{command}", command));
    let media = await m.quoted.download();
    let out = null;
    if (/webp/.test(mime)) {
  ...
  if (/^to(vn|ptt|voicenote)$/i.test(command)) {
    ...
    if (!media) return m.reply(global.settings.message.cantDownloadMedia);
    let audio = await toPTT(media, "mp4").catch(() => null);
    if (!audio) return m.reply(global.settings.message.cantConvertAudio);
