let handler = async (m, { sock, command, usedPrefix, text }) => {
  if (/add(msg|vn|video|audio|img|sticker|gif)/i.test(command)) {
    ...
    if (!text) return m.reply(`Usage: ${usedPrefix + command} <teks>\nExample:\n${usedPrefix + command} tes`);
  ...
  if (/del(msg|vn|video|audio|img|sticker|gif)/i.test(command)) {
    ...
    if (!global.db.data.msgs || !(text in global.db.data.msgs)) return m.reply(`Data \`${text}\` not found.`);
    delete global.db.data.msgs[text];
    global.db.write?.();
    await sock.sendReact(m.chat, "🗑️", m.key);
  }
  ...
  if (/get(msg|vn|video|audio|img|sticker|gif)/i.test(command)) {
    ...
    if (!global.db.data.msgs || !(text in global.db.data.msgs)) return m.reply(`'${text}' not listed in the message list`);
    await sock.copyNForward(m.chat, JSON.parse(JSON.stringify(global.db.data.msgs[text])), true).catch(() =>
      m.reply("Failed to send stored message (media may be expired)")
    );
  }

  if (/list(msg|vn|video|audio|img|sticker|gif)/i.test(command)) {
    let msgs = global.db.data.msgs || {};
    let list = Object.keys(msgs).map((v) => `- ${v}`).join("\n") || "- (empty)";
    m.reply(`*List Message*\n${list}\n─────────────────────\n> Get : \`\`\`.getmsg [text]\`\`\``);
  }
};
handler.help = ["addmsg", "delmsg", "getmsg", "listmsg"].map((v) => v + " <teks>");
handler.tags = ["tools"];
