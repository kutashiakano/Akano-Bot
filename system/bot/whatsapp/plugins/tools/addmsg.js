let handler = async (m, {sock: sock, command: command, usedPrefix: usedPrefix, text: text}) => {
  ...
  if (/get(msg|vn|video|audio|img|sticker|gif)/i.test(command)) {
    ...
    await sock.copyNForward(m.chat, JSON.parse(JSON.stringify(global.db.data.msgs[text])), true).catch(() => m.reply("🚩 Failed to send stored message (media may be expired)"));
  }
