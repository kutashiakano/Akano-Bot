...
const {applyEffect: applyEffect} = require("../../../discord/plugins/images/process");
...
async function imgBuf(sock, m, text) {
  ...
  if (urlMatch) {
    const {fImgBuf: fImgBuf} = require("../../../discord/plugins/images/process");
...
const __orig = {
  ...
  command: [ "invert", "grayscale", "sepia", "blur", "pixelate", "flip", "rotate", "contrast", "meme" ],
  tags: [ "tools" ],
  run: async (m, {sock: sock, text: text, command: command}) => {
    ...
    try {
      ...
      const options = {};
      if (fx.amount) {
        const num = parseInt(String(text || "").trim().split(/\s+/)[0], 10);
      ...
      if (fx.text) {
        ...
        if (pipeIndex > -1) {
          ...
          options.bottom = String(text).slice(pipeIndex + 1).trim();
        } else {
