function parseBlocks(text) {
  ...
  const pushText = (t) => {
    ...
    const flush = () => {
      if (cur.length) {
        blocks.push({ type: "text", text: cur.join("\n") });
        cur = [];
      }
    };
  ...
  while ((m = fenceRe.exec(text)) !== null) {
    ...
    blocks.push({ type: "code", lang: m[1] || "text", code: m[2] });
...
const __orig = {
  ...
  tags: ["ai"],
  run: async (m, { sock, text, usedPrefix, command }) => {
    ...
    switch (command) {
      ...
      case "gemini":
        ...
        try {
          ...
          const hasRich =
            blocks.some((b) => b.type === "code" || b.type === "table") ||
            /https?:\/\/\S+/i.test(replyText);

          if (hasRich && typeof sock.aiRich === "function") {
            const rich = sock
              .aiRich()
              .setTitle(options.model ? `Gemini ${options.model.model_name}` : "Gemini AI");
            for (const b of blocks) {
              if (b.type === "code") rich.addCode(b.lang || "text", b.code);
              else if (b.type === "table") rich.addTable(b.rows);
              else if (b.text.trim()) rich.addText(b.text);
            }
            ...
            for (const imgUrl of response.images) {
              try {
                await sock.sendMessage(
                  m.chat,
                  { image: { url: imgUrl }, caption: "Generated image" },
                  { quoted: m },
                );
              } catch (e) {}
            }
