let handler = async (m, {args: args, usedPrefix: usedPrefix, isOwner: _isOwner, isAdmin: _isAdmin}) => {
  ...
  if (!input) {
    const mode = chat.verifQuestion && chat.verifAnswer ? `Question: ${chat.verifQuestion}\nAnswer: ${chat.verifAnswer}` : "Current mode: random CAPTCHA";
    return m.reply(`Verification:\n${mode}\n\nSet a question: ${usedPrefix}setverif <question> | <answer>\nReset to CAPTCHA: ${usedPrefix}setverif reset`);
  }
