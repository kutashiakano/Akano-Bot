let handler = async (m, { sock, args, usedPrefix, command, isOwner: _isOwner, isAdmin: _isAdmin }) => {
  ...
  if (!global.db.data.chats[m.chat]) {
    global.db.data.chats[m.chat] = { welcome: true, left: true, detect: false, mute: false, member: [], chat: 0, expired: 0 };
  }
  ...
  if (!type) {
    const rows = [
      { title: "Welcome", description: chat.welcome ? "ON" : "OFF", id: `${usedPrefix}enable welcome` },
      { title: "Goodbye", description: chat.left ? "ON" : "OFF", id: `${usedPrefix}enable leave` },
      { title: "Detect", description: chat.detect ? "ON" : "OFF", id: `${usedPrefix}enable detect` },
      { title: "Set Welcome", description: chat.sWelcome ? "Custom" : "Default", id: `${usedPrefix}enable setwelcome ` },
      { title: "Set Goodbye", description: chat.sBye ? "Custom" : "Default", id: `${usedPrefix}enable setleave ` },
      { title: "Set Promote", description: chat.sPromote ? "Custom" : "Default", id: `${usedPrefix}enable setpromote ` },
      { title: "Set Demote", description: chat.sDemote ? "Custom" : "Default", id: `${usedPrefix}enable setdemote ` },
      { title: "Reset All", description: "Reset to default", id: `${usedPrefix}enable reset` },
    ];
    ...
    await sock.sendIAMessage(m.chat, [
      ...
      content: [
        ...
        `Goodbye: *${chat.left ? "ON" : "OFF"}*`,
        `Detect: *${chat.detect ? "ON" : "OFF"}*`,
        "",
        `Custom welcome: *${chat.sWelcome ? "Set" : "Default"}*`,
        `Custom goodbye: *${chat.sBye ? "Set" : "Default"}*`,
        `Custom promote: *${chat.sPromote ? "Set" : "Default"}*`,
        `Custom demote: *${chat.sDemote ? "Set" : "Default"}*`,
      ].join("\n"),
      footer: "Akano Bot",
    });
    return;
