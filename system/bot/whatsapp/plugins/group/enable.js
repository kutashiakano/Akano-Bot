let handler = async (
  m,
  { sock, args, usedPrefix, command, isOwner: _isOwner, isAdmin: _isAdmin },
) => {
    ...
    await sock.sendIAMessage(
      ...
      m,
      {
        ...
        content: [
          ...
          `Verification: *${chat.verification === false ? "OFF" : "ON"}*`,
          "",
          ...
          `Verification question: *${chat.verifQuestion ? "Set" : "Default (CAPTCHA)"}*`,
        ].join("\n"),
        footer: "Akano Bot",
      },
    );
  ...
  switch (type) {
    ...
    case "setwelcome":
      ...
      if (!welcomeText)
        return m.reply(`Usage: ${usedPrefix}enable setwelcome Welcome @user to @subject!`);
    ...
    case "setpromote":
      ...
      if (!promoteText)
        return m.reply(`Usage: ${usedPrefix}enable setpromote @user is now an admin!`);
    ...
    case "setdemote":
      ...
      if (!demoteText)
        return m.reply(`Usage: ${usedPrefix}enable setdemote @user is no longer an admin.`);
