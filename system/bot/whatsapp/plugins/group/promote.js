let handler = async (m, { sock, args, usedPrefix }) => {
  ...
  if (!target) return m.reply(`Usage: ${usedPrefix}promote @user or reply to user message`);
  
  const isAlready = participants.some(p => p.id === target && (p.admin === "admin" || p.admin === "superadmin"));
  if (isAlready) return m.reply("User is already an admin");
