...
function collectCommands(usedPrefix) {
  ...
  for (const pl of Object.values(global.plugin || {})) {
    ...
    const cats = Array.isArray(pl.category)
      ...
      : pl.tags
      ? (Array.isArray(pl.tags) ? pl.tags : [pl.tags])
      : []
...
module.exports = {
  ...
  run: async (m, { sock, usedPrefix, args }) => {
    ...
    const flamingBase =
      'https://www.flamingtext.com/net-fu/proxy_form.cgi?&imageoutput=true&script=sketch-name&doScale=true&scaleWidth=800&scaleHeight=500&fontsize=100&fillTextType=1&fillTextPattern=Warning!&fillColor1Color=%23b5b5b5&fillColor2Color=%23b5b5b5&fillColor3Color=%23b5b5b5&fillColor4Color=%23b5b5b5&fillColor5Color=%23b5b5b5&fillColor6Color=%23b5b5b5&fillColor7Color=%23b5b5b5&fillColor8Color=%23b5b5b5&fillColor9Color=%23b5b5b5&fillColor10Color=%23b5b5b5&fillOutlineColor=%23888888&fillOutline2Color=%23888888&backgroundColor=%23101820&text='
    ...
    const info = [
      ...
      `_Version_    : *v${version}*`,
      `_Uptime_     : *${process.uptime().toFixed(0)}s*`,
      `_Users_      : *${Object.keys(db.data.users).length}*`,
      `_Chats_      : *${Object.keys(db.data.chats).length}*`,
      `_Commands_   : *${totalCommands}*`,
      `_User_       : @${m.sender.split('@')[0]}`
    ].join('\n')
    ...
    if (args.length) {
      ...
      if (key === 'all') {
        ...
        return sock.sendMessage(
          m.chat,
          {
            location: { name: 'Akano Interactive', address: 'Simple WhatsApp Bot', jpegThumbnail: thumbAll },
            caption: `*[ ALL FEATURES ]*\n${allCmds}`,
            footer: 'Simple WhatsApp Bot',
            mentions: [m.sender],
            nativeFlow: [
              { name: 'quick_reply', buttonParamsJson: JSON.stringify({ display_text: 'Menu', id: '.menu' }) },
              { name: 'quick_reply', buttonParamsJson: JSON.stringify({ display_text: 'Donation', id: '.donation' }) }
            ],
      ...
      return sock.sendMessage(
        m.chat,
        {
          location: { name: 'Akano Interactive', address: 'Simple WhatsApp Bot', jpegThumbnail: thumb },
          caption: `*[ ${keyUpper} ]*\n${cmds}`,
          footer: 'Simple WhatsApp Bot',
          mentions: [m.sender],
          nativeFlow: [
            { name: 'quick_reply', buttonParamsJson: JSON.stringify({ display_text: 'Menu', id: '.menu' }) },
            { name: 'quick_reply', buttonParamsJson: JSON.stringify({ display_text: 'All', id: '.menu all' }) }
          ],
          interactiveAsTemplate: false
        },
        quoted
    ...
    await sock.sendMessage(
      m.chat,
      {
        location: { name: 'Akano Interactive', address: 'Simple WhatsApp Bot', jpegThumbnail: thumbmain },
        caption: info,
        footer: 'Simple WhatsApp Bot',
        mentions: [m.sender],
        nativeFlow: [
          { name: 'cta_url', buttonParamsJson: JSON.stringify({ display_text: '+ Owner', url: 'https://wa.me/' + (global.owner && global.owner[0] ? global.owner[0] : '628123456789'), merchant_url: '' }) },
          { name: 'single_select', buttonParamsJson: JSON.stringify({ title: 'Select Menu', sections: [{ title: 'Commands', highlight_label: '✨ All Features', rows }] }) }
        ],
        interactiveAsTemplate: false
      },
      quoted
