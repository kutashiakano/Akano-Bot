...
const cookiePath = path.join(import.meta.dirname, "../../../../../cookies.txt");
    ...
    try {
      ...
      if (isGallery) {
        ...
        if (isAudio && audioFiles.length > 0) {
          ...
          if (medias.length > 1) {
            await sock.sendAlbumMessage(m.chat, medias, {
              quoted: m,
              caption: albumCaption
            });
          } else {
            await sock.sendFile(m.chat, medias[0].data.url, path.basename(medias[0].data.url), albumCaption, m);
          }
        } else {
          throw new Error(`Failed to download TikTok ${isAudio ? "audio" : "carousel"} - no valid files.`);
        }
        ...
        await sock.sendFile(m.chat, file, path.basename(file), `*TikTok ${isAudio ? "Audio" : "Download"}*`, m);
        await downloader.cleanup(result.directory);
      }
