  ...
  run: async (m, { sock: sock, text: text }) => {
    ...
    try {
      ...
      if (isGallery) {
        ...
        const author = metadata[0]?.uploader || metadata[0]?.channel || metadata[0]?.creator || "Unknown";
        ...
        if (isAudio && audioFiles.length > 0) {
          ...
          throw new Error(`Failed to download Instagram ${isAudio ? "audio" : "carousel"} - no valid files.`);
        }
