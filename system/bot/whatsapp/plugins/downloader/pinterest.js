...
const __orig = {
        ...
        if (isAudio && audioFiles.length > 0) {
          ...
          if (medias.length > 1) {
            await sock.sndAlb(m.chat, medias, {
              quoted: m,
              caption: `*Pinterest Carousel* — ${medias.length} pins`,
            });
          } else {
            await sock.sendFile(
              m.chat,
  ...
  example:
    "%cmd https://www.pinterest.com/pin/1234567890\n%cmd https://www.pinterest.com/pin/1234567890 --audio",
}
const { define } = require("../../../plugin");

module.exports = define({
  name: ["pinterest", "pindl"],
  category: (["downloader"])[0] || "tools",
  help: (["pinterest", "pindl"])[0] || "",
  reg: true,
