let handler = async (m, { sock: sock, args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  if (command === "unreg") {
    if (!user || !user.registered) {
      return m.reply(global.settings.message.notRegistered);
    }
    user.registered = false;
    delete user.age;
    delete user.regTime;
    return m.reply(global.settings.message.unregDone);
  }
  if (user?.registered) {
    return m.reply(global.settings.message.alreadyRegistered + `\nUse ${usedPrefix}unreg to reset your registration.`);
  }
...
function sndAgeSel(sock, jid, { quoted: quoted, usedPrefix: usedPrefix, name: name }) {
  const row = age => ({
    title: `${age} Years`,
