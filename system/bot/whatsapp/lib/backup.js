...
class AutoBackup {
  constructor(dbPath, intervalMs = 36e5) {
    this.dbPath = dbPath;
  ...
  cleanOldBackups() {
    try {
      const files = fs.readdirSync(this.backupDir).filter(f => f.startsWith("backup-") && f.endsWith(".json")).sort().reverse();
      if (files.length > 7) {
  ...
  listBackups() {
    try {
      return fs.readdirSync(this.backupDir).filter(f => f.startsWith("backup-") && f.endsWith(".json")).sort().reverse().map(f => ({
        name: f,
