...
function getConfig() {
  ...
  return {
    ...
    expiresIn: Number(cfg.expiresIn) > 0 ? Number(cfg.expiresIn) : 5 * 60 * 1e3,
...
function sweepExpired() {
  ...
  for (const key of Object.keys(state)) {
    ...
    const isTerminal = [ STATUS.VERIFIED, STATUS.APPROVED, STATUS.FAILED, STATUS.EXPIRED ].includes(verification.status);
...
async function onJoinRequest(sock, {id: id, participant: participant, action: action}) {
  startSweeper();
  const config = getConfig();
  ...
  try {
    ...
    const found = metadata?.participants?.find(u => typeof u.id === "string" && sock.decodeJid(u.id) === userJid || typeof u.lid === "string" && u.lid === userJid);
  ...
  const useQuestion = typeof chat.verifQuestion === "string" && chat.verifQuestion.trim() && typeof chat.verifAnswer === "string" && chat.verifAnswer.trim();
  ...
  try {
    const minutes = Math.max(1, Math.round(config.expiresIn / 6e4));
    const text = useQuestion ? formatQuestionMessage(chat.verifQuestion.trim(), minutes) : formatCaptchaMessage(captcha, minutes);
...
async function handleReply(sock, m) {
  ...
  if (answer.toLowerCase() !== expected.toLowerCase()) {
    ...
    if (verification.attempts >= config.maxAttempts) {
      ...
      await persist();
      await sock.reply(m.chat, `Wrong code.\n\nAttempt: ${verification.attempts}/${config.maxAttempts}\nPlease reply to the verification message with the correct code.`, m).catch(() => {});
    }
    return true;
  }
  ...
  delete state[key];
  await persist();
  await sock.reply(m.chat, approved ? "Verification successful.\n\nYour join request has been approved." : "Verification failed.\n\nPlease send a new join request to get a new code.", m).catch(() => {});
  return true;
}
