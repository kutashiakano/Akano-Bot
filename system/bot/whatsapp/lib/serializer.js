const { proto, areJidsSameUser } = require("baileys");
...
function smsg(sock, m, hasParent) {
  ...
  if (m.key) {
    m.id = m.key.id;
    const isBotFn =
      global.settings.connection?.bot || ((id) => id.startsWith("BAE5") || id.indexOf("-") > 1);
    ...
    m.sender = sock.decodeJid(
      (m.key.fromMe && sock.user.id) || m.participant || m.key.participant || m.chat || "",
    );
  ...
  if (m.message) {
    ...
    if (typeof m.text !== "string") {
      if (
        ...
        m.text =
          m.text.selectedDisplayText || m.text.hydratedTemplate?.hydratedContentText || m.text;
      }
    }
    ...
    if (m.quoted) {
      ...
      m.quoted.mentionedJid =
        (m.quoted.contextInfo?.mentionedJid?.length && m.quoted.contextInfo.mentionedJid) || [];
      ...
      if (m.quoted.url || m.quoted.directPath) {
        m.quoted.download = (saveToFile = false) =>
          sock.downloadM(m.quoted, m.quoted.mtype.replace(/message/i, ""), saveToFile);
      }
      ...
      m.quoted.cMod = (jid, text = "", sender = m.quoted.sender, options = {}) =>
  ...
  m.forward = (jid = m.chat, forceForward = false, options = {}) =>
...
module.exports = { serializeM, smsg };
