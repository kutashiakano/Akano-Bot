...
function getMediaDuration(buffer, ext) {
  return new Promise((resolve) => {
    ...
    spawn("ffprobe", ["-v", "error", "-show_entries", "format=duration", "-of", "csv=p=0", tmpFile], { env: getSpawnEnv() })
      .on("error", () => { try { fs.unlinkSync(tmpFile); } catch {} resolve(6); })
    ...
    try {
      ...
      if (isAnimated) {
        ...
        args = [
          ...
          "-i", inputFile,
          "-vcodec", "libwebp",
          "-vf", "scale=512:512:force_original_aspect_ratio=decrease,fps=15,pad=512:512:-1:-1:color=0x00000000,split[a][b];[a]palettegen=reserve_transparent=on:transparency_color=000000[p];[b][p]paletteuse",
          "-loop", "0",
          "-t", String(Math.min(duration, 6)),
          "-preset", "default",
          "-an",
          "-pix_fmt", "yuva420p",
          tmpOut,
        ...
        args = [
          ...
          "-i", inputFile,
          "-vcodec", "libwebp",
          "-vf", "scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:-1:-1:color=0x00000000,split[a][b];[a]palettegen=reserve_transparent=on:transparency_color=000000[p];[b][p]paletteuse",
          "-lossless", "0",
          "-compression_level", "6",
          "-quality", "80",
          "-loop", "0",
