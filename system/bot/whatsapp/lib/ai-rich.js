...
const { prepareWAMessageMedia, wrapToBotForwardedMessage, tokenizeCode, proto } = require("baileys");
...
class AIRich extends BaseBuilder {
	...
	addText(text, { hyperlink = true, citation = true, latex = true } = {}) {
		...
		this._items.push({
			type: "text",
			text: extractedText,
			...(inline_entities.length && { inlineEntities: inline_entities }),
		});
	...
	addCode(language, code) {
		...
		this._items.push({ type: "code", language, code });
	...
	addSource(sources = []) {
		...
		for (const [icon, url, text] of sources) {
			this._links.push({
				...
				...(icon ? { sources: [{ source_type: "THIRD_PARTY", source_display_name: text || "Source", source_subtitle: "AI", source_url: url || "" }] } : {}),
			});
		}
	...
	addImage(imageUrl, { resolveUrl = false } = {}) {
		...
		this._items.push({ type: "image", image: imageUrl, resolveUrl });
	...
	addTip(text) {
		...
		this._items.push({ type: "tip", text });
	...
	addSuggest(suggestion, { scroll = true, layout } = {}) {
		...
		this._items.push({ type: "suggest", suggestion, scroll, layout });

		return this;
	}

	async build({ forwarded = true, quoted, quotedParticipant, ...options } = {}) {
		const uuid = crypto.randomUUID();
		const submessages = [];
		const sections = [];

		const pushTextSection = (text, inlineEntities = []) => {
			submessages.push({
				messageType: 2,
				messageText: text,
				...(inlineEntities.length ? { inlineEntities } : {}),
			});
			sections.push({
				view_model: {
					primitive: {
		...
		for (const item of this._items) {
			if (item.type === "text") {
				pushTextSection(item.text, item.inlineEntities || []);
					...
					view_model: {
						primitive: {
							...
							code_blocks: blocks.map((b) => ({
								...
								][b.highlightType] || "AI_RICH_RESPONSE_CODE_HIGHLIGHT_DEFAULT",
							})),
					...
					view_model: {
						primitive: {
							...
							rows: rows.map((row) => ({
								...
								cells: row.items,
								markdown_cells: row.items.map((cell) => ({ text: cell })),
							})),
				...
				pushTextSection(item.text);
				sections.push({
					view_model: {
						primitive: { text: item.text, __typename: "GenAIMetadataTextPrimitive" },
						__typename: "GenAISingleLayoutViewModel",
					},
				});
				...
				if (Buffer.isBuffer(item.image)) {
					const media = await prepareWAMessageMedia({ image: item.image }, { upload: this.#client.waUploadToServer, jid: "@newsletter" }).catch(() => null);
					url = media?.imageMessage?.url || null;
				}
				if (url) {
					submessages.push({
						messageType: 3,
						imageMetadata: { imageUrl: url, imageText: "", alignment: 0, tapLinkUrl: "" },
					});
					sections.push({
						view_model: {
							primitive: {
		...
		this._links.forEach((linkField, index) => {
			...
			const sources = linkField.sources?.length
				? linkField.sources.map((s) => ({
				...
				: [];
			pushTextSection(linkField.text + ` {{${prefix}}}¹{{/${prefix}}} `, [
				{
					key: prefix,
					metadata: {
		...
		if (this._body) {
			submessages.unshift({ messageType: 2, messageText: this._body });
			sections.unshift({
				view_model: {
					primitive: { text: this._body, __typename: "GenAIMarkdownTextUXPrimitive" },
					__typename: "GenAISingleLayoutViewModel",
				},
			});
		}
		if (this._footer) {
			submessages.push({ messageType: 2, messageText: this._footer });
			sections.push({
				view_model: {
					primitive: { text: this._footer, __typename: "GenAIMetadataTextPrimitive" },
					__typename: "GenAISingleLayoutViewModel",
				},
			});
		}
		...
		if (forwarded) {
			...
			contextInfo.forwardedAiBotMessageInfo = { botJid: "0@bot" };
		...
		const richResponseMessage = proto.AIRichResponseMessage.create({
			...
			submessages,
			unifiedResponse: {
				data: Buffer.from(JSON.stringify({ response_id: uuid, sections })),
			},
			contextInfo,
		});
	...
	async send(jid, { forwarded = true, quoted, quotedParticipant, ...options } = {}) {
		const msg = await this.build({ forwarded, quoted, quotedParticipant, ...options });
