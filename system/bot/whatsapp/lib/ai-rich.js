  ...
  async build({ forwarded = true, quoted, quotedParticipant, ...options } = {}) {
    ...
    for (const item of this._items) {
      if (item.type === "text") {
        ...
        const suggest = (Array.isArray(item.suggestion) ? item.suggestion : [item.suggestion]).map(
          (text) => ({
            prompt_text: text,
        ...
        const type =
          item.layout ?? (suggest.length === 1 ? "Single" : item.scroll ? "HScroll" : "ActionRow");
        sections.push({
          __typename: "GenAIUnifiedResponseSection",
          view_model: {
            [type === "Single" ? "primitive" : "primitives"]:
        ...
        if (Buffer.isBuffer(item.image)) {
          const media = await prepareWAMessageMedia(
            { image: item.image },
            { upload: this.#client.waUp, jid: "@newsletter" },
          ).catch(() => null);
