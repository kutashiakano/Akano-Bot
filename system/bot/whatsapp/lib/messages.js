...
function extMsgs(sock, store, ephemeral) {
  ...
  sock.sendMessageModifyV2 = async (jid, text, fakeTitle, options = {}, extra = {}) => {
    ...
    if (typeof fakeTitle === "string") {
      const thumb = await getThumb(path.join(import.meta.dirname, "../../../../media/image/thumb.jpg").replace(/\/+/g, "/"));
      let jpegThumb = null;
      try {
        const defPath = path.join(process.cwd(), "media/image/thumb.jpg");
        const tPath = fs.existsSync(defPath) ? defPath : path.join(import.meta.dirname, "../../../..", "media/image/default.jpg");
  ...
  sock.sendMessageVerify = async (jid, text, fakeName = "© Akano-Bot", options = {}) => {
    ...
    try {
      const defPath = path.join(process.cwd(), "media/image/thumb.jpg");
      const altPath = path.join(import.meta.dirname, "../../../../media/image/thumb.jpg");
      const tPath = fs.existsSync(defPath) ? defPath : fs.existsSync(altPath) ? altPath : path.join(import.meta.dirname, "../../../../media/image/default.jpg");
      if (fs.existsSync(tPath)) jpegThumb = await sock.resize(fs.readFileSync(tPath), 300, 300);
    } catch {}
    const fake = {
      key: {
