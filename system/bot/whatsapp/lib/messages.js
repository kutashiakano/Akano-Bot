...
`.trim();
  ...
  sock.sendIAMessage = async (jid, buttons, msg, options = {}) => {
    ...
    const nfButtons = (Array.isArray(buttons) ? buttons : [])
      ...
      .map((b) => {
        if (b.name && (b.btnJson || b.params)) {
          return { name: b.name, btnJson: b.btnJson || JSON.stringify(b.params) };
        }
        return {
          name: "quick_reply",
    ...
    let hdr = { title: header, hasMedia: false };
    ...
    if (media) {
      ...
      if (
        loc &&
        (loc.buffer || loc.image || loc.latitude !== undefined || loc.name || loc.address)
      ) {
