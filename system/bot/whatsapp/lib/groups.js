const {proto: proto, prepareMessageFromContent: prepareMessageFromContent} = require("baileys");

function extendGroups(sock) {
  sock.sendGroupV4Invite = async (jid, participant, inviteCode, inviteTtl, groupName = "unknown subject", caption = "Invitation to join my WhatsApp group", options = {}) => {
    let msg = proto.Message.fromObject({
      groupInviteMessage: proto.GroupInviteMessage.fromObject({
        inviteCode: inviteCode,
        inviteTtl: parseInt(inviteTtl) || +new Date(new Date + 3 * 864e5),
        groupJid: jid,
