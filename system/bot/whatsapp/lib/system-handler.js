...
const { Cooldown, SpamDetection, matcher, texted, toTime, generateLink, socmed } = require("./cooldown");
const { Notifier, getPermission, isOwner, isPremium, isAdmin, isBotAdmin } = require("./permission");
const { isLink, isVirtex, extractLinks, AntiDelete } = require("./anti");

const cooldown = new Cooldown(5000);
const spam = new SpamDetection({ RESET_TIMER: 5000, HOLD_THRESHOLD: 5, PERMANENT_THRESHOLD: 10, BANNED_THRESHOLD: 15 });
...
function initSystem(sock, global) {
  ...
  global.system = { cooldown, spam, antiDelete, notifier, texted, toTime };
}
...
async function systemHandler(sock, message, global) {
  ...
  for (const msg of messages) {
    ...
    if (isGroup && groupSet) {
      ...
      global.db.groupMessages[from].push({ key: msg.key, message: msg.message, sender, timestamp: Date.now() });
    ...
    if (isGroup && setting?.antilink !== false && groupSet?.antilink && !owner && !premium) {
      if (isLink(body)) {
        try {
          ...
          await sock.reply(from, `@${sender.split("@")[0]} links are not allowed!`, msg, { mentions: [sender] });
        } catch {}
        continue;
      }
    }
    ...
    if (isGroup && setting?.antivirtex !== false && groupSet?.antivirtex && !owner && !premium) {
      if (isVirtex(body)) {
        try {
          await sock.sendMessage(from, { delete: msg.key });
          await sock.reply(from, `@${sender.split("@")[0]} long text is not allowed!`, msg, { mentions: [sender] });
        } catch {}
        continue;
      }
    }
    ...
    if (cooldown.has(sender, command)) {
      ...
      await sock.reply(from, `Cooldown! Wait ${Math.ceil(remaining / 1000)}s`, msg);
      continue;
    }
    ...
    cooldown.set(sender, command, 5000);
...
function antiDeleteHandler(sock, update, global) {
...
function messageDeleteHandler(sock, update, global) {
