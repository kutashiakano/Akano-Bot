...
import { Cooldown, SpamDetection, matcher, texted, toTime, generateLink, socmed, isLink, isVirtex, extractLinks, AntiDelete, Notifier, getPermission } from "@kutashiakanocanzy/sdk";
...
async function systemHandler(sock, message, global) {
  ...
  for (const msg of messages) {
    ...
    if (!command) continue;
    const commands = [];
    const plugsDir = path.join(import.meta.dirname, "..", "plugins");
    try {
      const scanDir = async dir => {
        ...
        for (const file of files) {
          ...
          if (stat.isDirectory()) {
            ...
            try {
              const plug = await import(fullPath + "?t=" + Date.now());
              const mod = plug.default || plug;
              if (mod.help || mod.command) {
                const cmds = mod.help || mod.command || [];
...
export { initSystem, systemHandler, antiDel, delHandler };
