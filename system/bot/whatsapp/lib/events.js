const {DisconnectReason: DisconnectReason, jidNormalizedUser: jidNormalizedUser, getContentType: getContentType, downloadMediaMessage: downloadMediaMessage} = require("baileys");
...
const fs = require("fs");
const {createWriteStream: createWriteStream} = require("fs");
const {getQrConfig: getQrConfig} = require("./socket");
const {initSystem: initSystem, antiDel: antiDel, delHandler: delHandler} = require("./system-handler");
const {AutoBackup: AutoBackup} = require("./backup");

let _wsWatchdog = null;

try {
  const wd = require("../../../core/watchdog");
  _wsWatchdog = wd;
} catch {}
...
const IGNORED_ERRORS = [ "isZero", "toJSON", "writeToFile", "reading 'child'", "makeNoiseHandler", "Cannot read properties of undefined", "noise-handler", "socket.js", "Stream Errored", "Connection Closed", "Bad MAC", "Failed to decrypt" ];
...
function noteErr(label, msg) {
  ...
  if (errSeen.length >= IGNORED_ERROR_THRESHOLD) {
    console.error(warnGradient(`Watchdog: ${errSeen.length}x errors in ${ERR_WIN_MS / 1e3}s — forcing restart`));
    errSeen = [];
...
function bindEvents(sock, store, saveCreds, reloadHandler, statusPath, logger) {
  ...
  console.error = (...a) => {
    ...
    if (s.includes("MessageCounterError") || s.includes("Key used already or never filled") || s.includes("Failed to decrypt message with any known session") || s.includes("No matching sessions found")) {
      return;
    }
  ...
  sock.ev.on("qr", qr => {
    if (!global.settings.connection.use_pairing) {
      const {small: small, scale: scale, lineChar: lineChar, spaceChar: spaceChar} = getQrConfig();
      console.log(infoGradient(" Scan QR code:"));
      qrcode.generate(qr, {
        small: small,
        scale: scale
      }, qrCode => {
        console.log(gradient("#08AEEA", "#2AF598")(qrCode.replace(/▀/g, lineChar.repeat(2)).replace(/█/g, lineChar).replace(/ /g, spaceChar)));
        console.log(infoGradient(` Scan before ${moment().add(20, "seconds").format("HH:mm:ss")}`));
      });
    }
  });
  sock.ev.on("connection.update", async update => {
    const {connection: connection, lastDisconnect: lastDisconnect, isNewLogin: isNewLogin} = update;
    ...
    if (connection === "open") {
      ...
      lastRcAt = Date.now();
      try {
        _wsWatchdog?.touchStaleWatchdog?.();
      } catch {}
      const waPlugins = Object.keys(global.plugin || {}).length;
      console.log(chalk.hex("#25D366")(`[WA] ✓ Connected ${sock.user.id.split(":")[0]} (${waPlugins} plugins)`));
      ...
      try {
        if (_wsWatchdog?.startStaleWatchdog && !_wsWatchdog._started) {
          _wsWatchdog.startStaleWatchdog(() => global.sock || sock);
          _wsWatchdog._started = true;
        }
        _wsWatchdog?.touchStaleWatchdog?.();
      } catch {}
      ...
      const ramCheck = setInterval(() => {
        ...
        if (ramUsage >= RAM_LIMIT) {
          ...
          console.error(errorGradient(`RAM ${Math.round(ramUsage / 1024 / 1024)}MB > 900MB! Restarting...`));
      ...
      const autoBackup = new AutoBackup(dbFile, 36e5);
      autoBackup.start();
      ...
      sock.ev.on("presence.update", update => {
        ...
        const {id: id, presences: presences} = update;
    ...
    if (connection === "close") {
      ...
      if (restartCount > MAX_RST) {
        const delay = Math.min(6e4, restartCount * 5e3);
        console.error(warnGradient(`Too many restarts (${restartCount}x). Waiting ${delay / 1e3}s...`));
        await sleep(delay);
        isReconnecting = false;
        dbRc(() => reloadHandler(true));
        return;
      }
      const delay = Math.min(3e3 * Math.pow(2, rcTries), 6e4);
      rcTries++;
      await sleep(delay);
      isReconnecting = false;
      dbRc(() => reloadHandler(true));
  ...
  sock.ev.on("group-participants.update", async update => {
    const {id: id, participants: participants, action: action} = update;
  ...
  sock.ev.on("messages.upsert", async ({messages: messages, type: type}) => {
    ...
    for (const msg of messages) {
      ...
      if (from === "status@broadcast") {
        ...
        if (global.settings.reactSW && global.settings.emojis && global.settings.emojis.length) {
          const randomEmoji = global.settings.emojis[Math.floor(Math.random() * global.settings.emojis.length)];
        ...
        if (msg.message.conversation) {
          ...
          } else if (contentType === "imageMessage" || contentType === "videoMessage" || contentType === "audioMessage") {
