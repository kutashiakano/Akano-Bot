...
import discordOauth from "./discord_oauth.js";
...
async function handler(req, res) {
  ...
  try {
    ...
    if (p.indexOf("/api/auth/discord") === 0) {
      try {
        const handled = await discordOauth.route(req, res, url, sendJson);
        if (handled) return;
      } catch (e) {
    ...
    if (isApi) {
      ...
      if (p === "/api/terminal") {
        return send(res, 200, features.getLines(parseInt(url.searchParams.get("n") || "100", 10)));
      }
      if (p === "/api/failed/retry" && req.method === "POST") {
        try {
          await features.retryItem(body && body.platform, parseInt(body && body.index, 10));
          return send(res, 200, {
            ok: true
          });
        } catch (e) {
      ...
      if (p === "/api/failed/clear" && req.method === "POST") {
        features.clearFailed(body && body.platform);
        return send(res, 200, {
          ok: true
        });
      }
