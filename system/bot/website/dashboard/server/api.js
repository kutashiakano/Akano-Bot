...
import { snapshot, stats, topCommandsList, clearBuffers, pushMessage } from "./bus.js";
import busMod from "./bus.js";
import { redactText } from "./redact.js";
import store from "./store.js";
import featuresDefault from "./features.js";
...
let _PN = null;
import("awesome-phonenumber").then(m => {
  _PN = m.default ?? m;
}).catch(() => {});
...
let pkgVersion = "?";
try {
  pkgVersion = JSON.parse(fs.readFileSync(PKG_FILE, "utf8")).version || "?";
} catch {}
...
function syncDiscordPresence(presenceOverride) {
  try {
    ...
    let ActivityType;
    try {
      ({ ActivityType: ActivityType } = _sdkEngine() || {});
    } catch {
      ActivityType = {
        Playing: 0,
        Streaming: 1,
    ...
    let dbPresence = {};
    try {
      dbPresence = database.get().settings?.discord?.presence || {};
    } catch {}
...
function collectGroups() {
  ...
  try {
    const servers = database.get().discord?.servers || {};
  ...
  try {
    const chats = database.get().chats || {};
...
async function handleAction(req, res, url, body, send) {
  ...
  if (p === "/api/settings/save") {
    try {
      ...
      try {
        busMod.emitSettingsUpdate(merged);
      } catch {}
      return send(res, 200, {
        ok: true,
  ...
  if (p === "/api/settings/reload") {
    ...
    try {
      ...
      await import(pathToFileURL(settingsPath).href + "?t=" + Date.now());
    } catch (e) {
      return send(res, 500, {
        ok: false,
    ...
    try {
      busMod.emitSettingsUpdate(store.getOverlay());
    } catch {}
    return send(res, 200, {
      ok: true,
  ...
  if (p === "/api/dev/write") {
    ...
    const syntaxErr = (await import("syntax-error")).default(content, path.basename(full));
    if (syntaxErr) {
      return send(res, 400, {
        ok: false,
  ...
  if (p === "/api/music/volume") {
    const gid = String(body && body.guildId || "");
    let vol = parseInt(body && body.volume, 10);
    if (!gid) return send(res, 400, {
      ok: false,
      message: "guildId required"
    });
    if (isNaN(vol)) return send(res, 400, {
      ok: false,
    ...
    try {
      const playCmd = global.discordCommands && global.discordCommands.p;
      const qs = playCmd && typeof playCmd.getQueues === "function" ? playCmd.getQueues() : null;
      const q = qs ? qs.get(gid) : null;
      if (!q) return send(res, 404, {
        ok: false,
        message: "No active session"
      });
      q.volume = vol / 100;
      if (q.currentResource && q.currentResource.volume && typeof q.currentResource.volume.setVolume === "function") {
...
export { handleApi, handleAction, botStatus, applyOverlayToRuntime, json, botAvatar };
export default {
  handleApi: handleApi,
