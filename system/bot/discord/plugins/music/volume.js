const {createVolumeBar: createVolumeBar, formatDuration: formatDuration} = require("./utils");
...
module.exports = define({
  ...
  run: async ctx => {
    ...
    await interaction.editReply({
      embeds: [ (new interaction.client.ebuilder).setColor("#57F287").setTitle("Volume Set").setDescription(`${bar} \`${level}%\``) ]
    });
  }
});