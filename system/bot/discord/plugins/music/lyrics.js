...
function cleanLyricsText(text) {
  if (!text) return null;
  return text.replace(/\d+\s+Contributors.*?Lyrics(<[^>]+>)*\s*/is, "").replace(/<[^>]*>/g, "").replace(/^[^\[]+?\.{3}\s*Read More\s*/im, "").replace(/\n{3,}/g, "\n\n").trim() || null;
}
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data && data.length > 0) {
        ...
        setTimeout(() => lyricsCache.delete(cacheKey), 36e5);
        return result;
      }
    }
  } catch (e) {}
...
export { lyricsCache, fetchLyrics, fetchGeniusLyrics, lyrAny, cleanLyricsText };
export default { lyricsCache, fetchLyrics, fetchGeniusLyrics, lyrAny, cleanLyricsText };