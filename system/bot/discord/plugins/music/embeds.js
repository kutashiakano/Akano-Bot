const { formatDuration, createVolumeBar, bar, AUDIO_FILTERS } = require("./utils");
...
function controlButtons(queue) {
  const { abuilder, bbuilder, ButtonStyle } = CB(queue);
  const toggling = queue.paused;
  const toggle = icBtn(queue, toggling ? "play" : "pause", toggling ? "Play" : "Pause")
    .setCustomId("music_pause")
    .setStyle(toggling ? ButtonStyle.Success : ButtonStyle.Primary);
...
function genreButtons(client, genres) {
  const { abuilder, bbuilder, ButtonStyle } = client || {};
  const rows = [];
...
function nowPlaying(queue, elapsed) {
  ...
  const filterText =
    queue.currentFilter && queue.currentFilter !== "none"
      ? AUDIO_FILTERS[queue.currentFilter]?.label || queue.currentFilter
      : "None";
...
function queued(queue, song, position, formatDurationFn) {
  ...
  return new ebuilder()
    ...
    .addFields(
      { name: "Artist", value: `\`${song.uploader || "Unknown"}\``, inline: true },
      { name: "Duration", value: `\`[${formatDurationFn(song.duration)}]\``, inline: true },
