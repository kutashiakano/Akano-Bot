const { queues, playNext } = require("./engine");
...
function dashboardEmbed(guild, queue, hasSession) {
  ...
  if (now) {
    ...
    embed.setDescription(
      hasSession
        ? "Signed in to YouTube Music — explore your library, playlists, or search."
        : "Search YouTube Music or sign in via /account to unlock your library."
    );
  }
  embed.setFooter({ text: hasSession ? "Signed in • Pick an action below" : "Guest • /account to sign in" });
  return embed;
}

function dashboardComponents(guild) {
  const { abuilder, mbuilder } = guild.client;
  const select = new mbuilder()
    .setCustomId("music_dash_sel")
    .setPlaceholder("Choose an action")
    .addOptions(
      { label: "Search", value: "search", description: "Search YouTube Music", emoji: "🔍" },
      { label: "Recommendations", value: "recs", description: "Charts, moods & radio" },
      { label: "My Playlists", value: "playlists", description: "Browse your playlists" },
      { label: "Liked Songs", value: "liked", description: "Your liked songs" },
      { label: "Queue", value: "queue", description: "View current queue" },
      { label: "Account", value: "account", description: "Manage YouTube Music account" }
    );
...
async function handleSearch(interaction) {
  ...
  const input = new (interaction.client.textInput)()
    .setCustomId("music_search_q")
    .setLabel("Song, artist or album")
    .setStyle(interaction.client.TextInputStyle.Short)
    .setPlaceholder("e.g. YOASOBI - Idol")
    .setRequired(true)
    .setMaxLength(100);
  ...
  if (!q) {
    await submitted.reply({ content: "Empty query.", flags: 64 }).catch(() => {});
    return;
  }
  ...
  if (!results.length) {
    await submitted.editReply({ content: "No results for `" + q.replace(/`/g, "") + "`" }).catch(() => {});
    return;
  }
  ...
  const row = new (submitted.client.abuilder)().addComponents(
    new (submitted.client.mbuilder)().setCustomId("music_search_pick").setPlaceholder("Choose a song").addOptions(opts)
  );
  const embed = new (submitted.client.ebuilder)()
    .setColor("#5865F2")
    .setTitle("Results for " + q)
    .setDescription(results.map((r, i) => `\`${i + 1}.\` **${r.title}** — ${r.artist} \`${formatDuration(r.duration)}\``).join("\n"));
  const msg = await submitted.editReply({ embeds: [embed], components: [row] }).catch(() => null);
  if (!msg) return;
...
async function handleRecs(interaction) {
  const row = new (interaction.client.abuilder)().addComponents(
    new (interaction.client.bbuilder)().setCustomId("music_recs_charts").setLabel("Charts").setStyle(interaction.client.ButtonStyle.Secondary),
    new (interaction.client.bbuilder)().setCustomId("music_recs_moods").setLabel("Moods").setStyle(interaction.client.ButtonStyle.Secondary),
    new (interaction.client.bbuilder)().setCustomId("music_recs_radio").setLabel("Radio").setStyle(interaction.client.ButtonStyle.Primary)
  );
  const embed = new (interaction.client.ebuilder)()
    .setColor("#5865F2")
    .setTitle("Recommendations")
    .setDescription("Pick a source:");
...
async function handlePlaylists(interaction) {
  ...
  if (!ys.has(uid)) {
    await interaction.reply({ content: "Sign in via /account first to see your playlists.", flags: 64 }).catch(() => {});
    return;
  }
  ...
  col.on("collect", async (sel) => {
    ...
    const tEmbed = new (sel.client.ebuilder)()
      .setColor("#5865F2")
      .setTitle("Playlist • " + (plists.find((p) => p.id === pid)?.title || pid))
      .setDescription(tracks.length ? tracks.slice(0, 10).map((t, i) => `\`${i + 1}.\` ${t.title}`).join("\n") : "Empty playlist.");
    await sel.update({ embeds: [tEmbed], components: [tRow] }).catch(() => {});
  });
}
...
async function handleLiked(interaction) {
  const uid = interaction.user.id;
  if (!ys.has(uid)) {
    await interaction.reply({ content: "Sign in via /account first.", flags: 64 }).catch(() => {});
    return;
  }
  const liked = await ys.likes(uid, 25).catch(() => []);
  if (!liked.length) {
    await interaction.reply({ content: "No liked songs yet.", flags: 64 }).catch(() => {});
    return;
  }
  const row = new (interaction.client.abuilder)().addComponents(
    new (interaction.client.mbuilder)()
      .setCustomId("music_liked_pick")
      .setPlaceholder("Pick a liked song to play")
      .addOptions(liked.slice(0, 25).map((s) => ({ label: s.title.slice(0, 80), value: s.id })))
  );
  const embed = new (interaction.client.ebuilder)()
    .setColor("#5865F2")
    .setTitle("Liked Songs")
    .setDescription(liked.map((s, i) => `\`${i + 1}.\` ${s.title}`).join("\n").slice(0, 3800));
  ...
  col.on("collect", async (sel) => {
    ...
    if (!item) return sel.update({ content: "Expired.", embeds: [], components: [] }).catch(() => {});
    await sel.deferUpdate().catch(() => {});
    const play = require("./play.js");
    const res = await play.playPanel(sel, { videoId: vid, title: item.title });
    if (res.error) await sel.followUp({ embeds: [res.error], flags: 64 }).catch(() => {});
    else if (res.embed) await sel.followUp({ embeds: [res.embed] }).catch(() => {});
  });
}
...
async function handleQueue(interaction) {
  ...
  if (!q || (!q.currentSong && q.songs.length === 0)) {
    await interaction.reply({ content: "Queue is empty.", flags: 64 }).catch(() => {});
    return;
  }
  ...
  const embed = new (interaction.client.ebuilder)()
    .setColor("#5865F2")
    .setTitle("Queue")
    .setDescription(now + list)
    .setFooter({ text: `${q.songs.length} in queue • ${q.loop} • ${q.shuffle ? "shuffle on" : "shuffle off"}` });
...
async function execute(interaction) {
  ...
  const msg = await interaction.reply({ embeds: [embed], components: rows, flags: 64 }).catch(() => null);
  if (!msg) return;
  const fetched = await interaction.fetchReply().catch(() => msg);
  const col = fetched.createMessageComponentCollector({
  ...
  btnCol.on("collect", async (btn) => {
    ...
    if (id.startsWith("music_pl_play:")) {
      ...
      if (!tracks.length) return btn.reply({ content: "Empty.", flags: 64 }).catch(() => {});
      let q = queues.get(btn.guildId);
      if (!q) {
        const vc = btn.member?.voice?.channel;
        if (!vc) return btn.reply({ content: "Join a voice channel.", flags: 64 }).catch(() => {});
        q = await require("./engine").mkQueue(btn.guildId, vc, btn.channel);
      }
      for (const t of tracks) q.songs.push({ id: t.id, title: t.title, url: `https://music.youtube.com/watch?v=${t.id}`, duration: 0, uploader: "YouTube Music", requester: `<@${btn.user.id}>`, requesterId: btn.user.id, source: "YouTubeMusic" });
      svState(btn.guildId, q);
      await btn.reply({ content: `Added ${tracks.length} track(s) to queue.`, flags: 64 }).catch(() => {});
      if (!q.currentSong) {
      ...
      const pid = id.split(":")[1];
      const tracks = await ys.plist(btn.user.id, pid, 25).catch(() => []);
      const embed = new (btn.client.ebuilder)()
        .setColor("#5865F2")
        .setTitle("Tracks")
        .setDescription(tracks.map((t, i) => `\`${i + 1}.\` ${t.title}`).join("\n").slice(0, 3800) || "Empty");
      await btn.reply({ embeds: [embed], flags: 64 }).catch(() => {});
    }
  });
}
