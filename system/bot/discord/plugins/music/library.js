...
const KINDS = { y: "Search", p: "Playlists", l: "Liked", k: "Quick Picks" };
const KINDS2 = { search: "y", playlists: "p", liked: "l", picks: "k" };
const views = new Map();
const PER = 10;
...
async function tvList(kind, uid, limit = PER) {
  if (kind === "p")
    return (await ys.plists(uid, limit)).map((p) => ({
      id: p.id,
      title: p.title,
      artist: "Playlist",
      type: "playlist",
  ...
  if (kind === "l")
    return (await ys.likes(uid, limit)).map((s) => ({
      id: s.id,
      title: s.title,
      artist: "?",
      type: "song",
  ...
  if (kind === "k")
    return (await ys.charts(10)).map((c) => ({
      id: c.id,
      title: c.title,
      artist: "Chart",
      type: "playlist",
...
async function fetch(kind, uid, q, type) {
  ...
  const list =
    kind !== "y" && ys.clientOf(uid) !== "WEB"
      ? await tvList(kind, uid)
      : await search(kind, q, type);
  views.set(uid, [...(views.get(uid) || []).filter((v) => v.kind !== kind), { kind, list }]);
  return list;
}
...
function view(client, kind, list, off = 0) {
    ...
    .setDescription(
      page.length
        ? page
            .map(
              (x, i) =>
                `${String(off + i + 1).padStart(2, "0")}. **${x.title || x.name}** — ${x.artist || "?"}`,
            )
            .join("\n")
        : "Nothing here.",
    )
    .setFooter({ text: `Page ${off / PER + 1}` });
  const rows = [];
  if (page.length) {
    rows.push(
      new (client.abuilder)().addComponents(
        page.slice(0, 5).map((_, i) =>
          new (client.bbuilder)()
            .setStyle(client.ButtonStyle.Primary)
            .setLabel(String(off + i + 1))
            .setCustomId(`lb|${kind}|${off + i}`),
        ),
      ),
    );
    rows.push(
      new (client.abuilder)().addComponents(
        page.slice(5, 10).map((_, i) =>
          new (client.bbuilder)()
            .setStyle(client.ButtonStyle.Primary)
            .setLabel(String(off + i + 6))
            .setCustomId(`lb|${kind}|${off + i + 5}`),
        ),
      ),
    );
  }
  if (list.length > PER) {
    rows.push(
      new (client.abuilder)().addComponents(
        new (client.bbuilder)()
          .setStyle(client.ButtonStyle.Secondary)
          .setLabel("◀")
          .setCustomId(`lb|n|${kind}|b`)
          .setDisabled(off === 0),
        new (client.bbuilder)()
          .setStyle(client.ButtonStyle.Secondary)
          .setLabel("▶")
          .setCustomId(`lb|n|${kind}|f`)
          .setDisabled(off + PER >= list.length),
      ),
    );
  }
...
async function play(clk, i) {
  ...
  if (!item) {
    return clk.reply({ content: "Item expired - run the command again.", flags: 64 });
  }
  try {
    if (item.type === "playlist" || item.type === "album") {
      ...
      return clk.update({ content: msg, embeds: [], components: [] });
    }
    if (item.type === "artist") {
  ...
  options: [
    {
      ...
      type: 1,
      options: [
        { name: "q", type: 3, description: "Search query", required: true },
        {
          name: "type",
          type: 3,
    ...
    { name: "playlists", description: "Browse your YouTube Music playlists", type: 1 },
    { name: "liked", description: "Browse your liked songs", type: 1 },
    { name: "picks", description: "Quick picks personalized for your account", type: 1 },
  ],
  async execute(interaction) {
    ...
    try {
      ...
      if (sub === "search") {
        ...
        if (!q)
          return interaction.reply({ content: "Provide a search query.", flags: 64 });
      }
