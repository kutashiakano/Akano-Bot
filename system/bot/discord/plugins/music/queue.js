...
module.exports = define({
  ...
  run: async ctx => {
    ...
    const songList = queue.songs.slice(start, end).map((s, i) => `\`${start + i + 1}.\` ${s.title} \`${formatDuration(s.duration)}\` — ${s.requester}`).join("\n") || "*No songs in queue*";
    ...
    const embed = (new EmbedBuilder).setColor("#5865F2").setTitle(`Queue — ${interaction.guild.name}`).setDescription(`**Now Playing:**\n[${queue.currentSong.title}](${queue.currentSong.url}) \`${formatDuration(queue.currentSong.duration)}\`\n\n**Queue:**\n${songList}`).addFields({
      name: "Total Songs",
      ...
      text: `Page ${actualPage}/${totalPages}`
    });
