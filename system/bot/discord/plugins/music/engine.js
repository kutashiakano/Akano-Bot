import { joinVoiceChannel, getVoiceConnection, createAudioPlayer, createAudioResource, AudioPlayerStatus, VoiceConnectionStatus, entersState, NoSubscriberBehavior, StreamType } from "@discordjs/voice";
import fs from "node:fs";
import path from "node:path";
import dns from "node:dns";
import { formatDuration, parseSeekTime, AUDIO_FILTERS } from "./utils.js";
import { controlButtons, genreButtons, nowPlaying, queued } from "./embeds.js";
import { lyrAny } from "./lyrics.js";
import { AUTOPLAY_GENRES, smartRec, autoFill } from "./autoplay.js";
import emojis from "./emojis.js";
import ys from "../../../../scrapers/src/ytsession.js";
import ytmusic from "../../../../scrapers/src/ytmusic.js";
import { svState, ldState, clrState, ldAllState } from "./state.js";

dns.setDefaultResultOrder("ipv4first");

let _opus = null;

async function hasOpus() {
  if (_opus !== null) return _opus;
  try {
...
async function onMusicBtn(q, guildId, btn, message) {
  ...
  if (btn.customId === "music_pause") {
    ...
    for (const key of filterKeys) {
      ...
      filterRow.addComponents((new (B(q).bbuilder)).setCustomId(`filter_${key}`).setLabel(f.label).setStyle(q.currentFilter === key || !q.currentFilter && key === "none" ? B(q).ButtonStyle.Success : B(q).ButtonStyle.Secondary));
    }
    try {
      await btn.followUp({
    ...
    try {
      await btn.followUp({
        content: global.settings.message.fetchingLyrics,
        flags: 64
      });
    ...
    try {
      const { default: acc } = await import("../tools/account.js");
