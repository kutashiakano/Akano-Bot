...
const { queues, mkQueue, playNext } = require("./engine");
...
async function chartsView(interaction) {
  ...
  const rows = [];
  if (list.length) rows.push(sel(interaction.client, "ym_chart", list.map((c) => ({ label: c.title, value: c.id })), "Pick a chart to play…"));
  await interaction.editReply({ embeds: [emb], components: rows });
  const msg = await interaction.fetchReply();
  const col = msg.createMessageComponentCollector({ filter: (i) => i.customId === "ym_chart", time: TTL });
  col.on("collect", async (i) => {
    col.stop();
    const id = i.values?.[0];
    if (!id) return i.update({ content: "Nothing selected.", embeds: [], components: [] }).catch(() => {});
    const pl = list.find((c) => c.id === id);
    const r = await playCmd.playPanel(i, { playlistId: id });
    let content = "Playing your chart.";
    if (r && r.error) {
      await i.update({ content: "Chart can't be played: the chart may require login.", embeds: [], components: [] }).catch(() => {});
      return;
    }
    if (pl) content = `Playing chart: **${pl.title}**`;
    await i.update({ content, embeds: [], components: [] }).catch(() => {});
  });
}
...
async function moodsView(interaction) {
  ...
  if (!list.length) {
    return interaction.editReply({ embeds: [lEmb(interaction.client, "Moods & Genres", [], "Could not load moods.")] });
  }
  const emb = lEmb(interaction.client, "Moods & Genres - YouTube Music", list.map((m) => m.title), "Nothing here.");
  const links = new Map(list.map((m) => [m.browseId, { params: m.params, title: m.title }]));
  pending.set(interaction.user.id, links);
  await interaction.editReply({ embeds: [emb], components: [sel(interaction.client, "ym_mood", list.map((m) => ({ label: m.title, value: m.browseId })), "Pick a mood…")] });
  const msg = await interaction.fetchReply();
  const col = msg.createMessageComponentCollector({ filter: (i) => /^ym_mood/.test(i.customId), time: TTL });
  col.on("collect", async (i) => {
    const val = i.values?.[0];
    if (i.customId === "ym_mood") {
      const mod = pending.get(interaction.user.id);
      if (!mod || !val) return;
      const info = mod.get(val) || { params: "", title: "Playlists" };
      const pls = await ys.moodPls(val, info.params, 25);
      if (!pls.length) {
        await i.update({ content: "This mood has no playlists.", embeds: [], components: [] }).catch(() => {});
        return;
      }
      const embPl = lEmb(interaction.client, "Playlists › " + info.title, pls.map((p) => p.title), "Nothing here.");
      await i.update({ embeds: [embPl], components: [sel(interaction.client, "ym_moodpl", pls.map((p) => ({ label: p.title, value: p.id })), "Pick a playlist…")] }).catch(() => {});
      return;
    }
    if (i.customId !== "ym_moodpl") return;
    col.stop();
    if (!val) return;
    await i.update({ content: "Loading playlist…", embeds: [], components: [] }).catch(() => {});
    const r = await playCmd.playPanel(i, { playlistId: val });
    if (r && r.error) {
      await interaction.editReply({ content: "Could not play that playlist.", embeds: [], components: [] }).catch(() => {});
    }
  });
}
...
async function radioView(interaction, query) {
  ...
  await interaction.editReply({ content: "Radio from **" + (tracks[0].title || "your song") + "**: " + msg, embeds: [], components: [] });
}
...
module.exports = define({
  ...
  options: [
    { name: "charts", type: 1, description: "Browse the top charts on YouTube Music" },
    { name: "moods", type: 1, description: "Browse moods & genres and play a playlist" },
    {
      name: "radio",
      type: 1,
      description: "Start a YouTube Music radio from the current song or a query",
      options: [
        { name: "query", type: 3, description: "Optional song or artist to start a radio from", required: false },
      ],
    },
  ],
  run: async (ctx) => {
