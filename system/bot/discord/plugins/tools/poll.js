import { discord as dcSdk } from "@kutashiakanocanzy/sdk";
...
export default define({
  ...
  run: async ctx => {
    ...
    try {
      ...
      const options = [ interaction.options.getString("option1"), interaction.options.getString("option2"), interaction.options.getString("option3"), interaction.options.getString("option4") ].filter(Boolean).map(t => t.trim()).filter(Boolean);
      ...
      const built = dcSdk.poll(question, options.map(t => t.slice(0, 55)), { duration: hours, multiselect: allowMultiselect });
      await interaction.reply({
        poll: typeof built?.toJSON === "function" ? built.toJSON() : built
      });
    } catch (e) {
