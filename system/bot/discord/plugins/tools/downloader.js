...
function compressAudio(file, maxSize) {
  return new Promise((resolve) => {
    ...
    const tmpOut = path.join(
      os.tmpdir(),
      `akano_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.mp3`,
    );
    execFile(
      "ffmpeg",
      ["-y", "-i", file, "-b:a", "64k", "-ac", "1", tmpOut],
      { timeout: 120000 },
      async (err) => {
...
function buildEmbed(info, source, interaction) {
  ...
  if (source === "tiktok") {
    ...
    embed.setTitle(
      `Instagram ${isCarousel ? "Carousel (" + info._raw.length + " items)" : "Post"}`,
    );
    if (author !== "Unknown") embed.addFields({ name: "Author", value: author, inline: true });
    if (desc) {
      const shortDesc = desc.length > 300 ? desc.substring(0, 300) + "..." : desc;
  ...
  run: async (ctx) => {
    ...
    try {
      ...
      if (isGallery && validFiles.length > 1) {
        ...
        const finalEmbed = new (interaction.client.ebuilder)()
          .setColor("#57F287")
