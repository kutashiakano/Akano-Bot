...
export default define({
  ...
  run: async ctx => {
    ...
    const embed = card().setColor(member?.displayHexColor || "#5865F2").setTitle(`${targetUser.username}${targetUser.discriminator !== "0" ? "#" + targetUser.discriminator : ""}`).setThumbnail(targetUser.displayAvatarURL({
      dynamic: true,
      size: 256
    })).addFields({
      name: "ID",
