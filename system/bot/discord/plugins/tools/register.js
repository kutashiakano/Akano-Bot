...
let shieldBuf = null;
function shieldFile() {
  if (shieldBuf) return [{ attachment: shieldBuf, name: "shield.png" }];
  try {
    shieldBuf = fs.readFileSync(SHIELD_PNG);
    return [{ attachment: shieldBuf, name: "shield.png" }];
  } catch (e) {
    return [];
  }
}
...
const cmd = define({
  ...
  run: async ctx => {
    ...
    await interaction.editReply({ embeds: [e], files: shieldFile(), components: [ageRow(interaction.client)] });
  }
});
