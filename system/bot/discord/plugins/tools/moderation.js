...
function dbMod(userId) {
  try {
    const db = database.get();
    if (!db.discord) db.discord = {};
    if (!db.discord.users) db.discord.users = {};
    const u = db.discord.users[String(userId)] || (db.discord.users[String(userId)] = {});
    if (!u.mod || typeof u.mod !== "object") u.mod = { warns: [], violations: 0, lastReason: "", lastAt: 0 };
    if (!Array.isArray(u.mod.warns)) u.mod.warns = [];
    return { db: db, mod: u.mod };
  } catch (e) {
    return { db: null, mod: { warns: [] } };
  }
}

function getWarnings(guildId, userId) {
  const key = `${guildId}:${userId}`;
  const mem = warnDB.get(key) || [];
  try {
    const stored = dbMod(userId).mod.warns || [];
    const seen = new Set(mem.map(w => w.timestamp + "|" + w.reason));
    const merged = mem.concat(stored.filter(w => !seen.has(w.timestamp + "|" + w.reason)));
...
function addWarning(guildId, userId, reason, moderator) {
  ...
  try {
    const r = dbMod(userId);
    r.mod.warns.push(entry);
    if (r.mod.warns.length > 25) r.mod.warns = r.mod.warns.slice(-25);
    if (r.db) database.write(r.db).catch(() => {});
  } catch (e) {}
  return warnings;
}
