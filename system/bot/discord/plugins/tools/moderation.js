...
function dmEmbed(client, type, guildName, reason) {
  ...
  return new (interaction.client.ebuilder)()
    .setColor("#ED4245")
...
module.exports = define({
  ...
  options: [
    ...
    },
    {
      ...
      options: [{ name: "user_id", type: 3, description: "ID of user to unban", required: true }],
    },
    {
      name: "timeout",
      type: 1,
  ...
  run: async (ctx) => {
    const interaction = ctx.interaction;
        ...
        if (sub === "kick") {
          ...
          if (!target.kickable) {
            return interaction.editReply({ embeds: [errEmbed(interaction.client, "Bot cannot kick this user.")] });
          }
          try {
            await target.send({ embeds: [dmEmbed(target.client, "kick", guild.name, reason)] }).catch(() => {});
            ...
            return interaction.editReply({ embeds: [errEmbed(interaction.client, "🚩 Failed to kick: " + e.message)] });
          }
        }
        ...
        if (sub === "ban") {
          ...
          if (!target.bannable) {
            return interaction.editReply({ embeds: [errEmbed(interaction.client, "Bot cannot ban this user.")] });
          }
          try {
            await target.send({ embeds: [dmEmbed(target.client, "ban", guild.name, reason)] }).catch(() => {});
            ...
            return interaction.editReply({ embeds: [errEmbed(interaction.client, "🚩 Failed to ban: " + e.message)] });
          }
        }
        ...
        if (sub === "unban") {
          ...
          try {
            ...
            return interaction.editReply({ embeds: [errEmbed(interaction.client, "🚩 Failed to unban: " + e.message)] });
          }
        }
        ...
        if (sub === "timeout") {
          ...
          try {
            ...
            return interaction.editReply({ embeds: [errEmbed(interaction.client, "🚩 Failed to timeout: " + e.message)] });
          }
        }
        ...
        if (sub === "clear") {
          ...
          try {
            ...
            const filtered = toDelete.filter(
