  ...
  run: async ctx => {
    ...
    if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
      return interaction.editReply({
        embeds: [ card().setColor("#ED4245").setDescription("You don't have permission to Manage Server.") ]
      });
    }
    ...
    const embed = card().setColor(color).setTitle(title).setDescription(message).setFooter({
    ...
    try {
      ...
      await interaction.editReply({
        embeds: [ card().setColor("#57F287").setDescription(`Announcement sent successfully to <#${targetChannel.id}>.`) ]
      });
    } catch (e) {
      await interaction.editReply({
        embeds: [ card().setColor("#ED4245").setDescription(`Failed to send announcement: ${e.message}`) ]
      });
    }
  }
});
