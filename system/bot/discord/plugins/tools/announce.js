...
module.exports = define({
  ...
  run: async (ctx) => {
    const interaction = ctx.interaction;
        ...
        const { EmbedBuilder, PermissionFlagsBits } = interaction.client;
        try {
        ...
        if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
          return interaction.editReply({
            embeds: [
              new EmbedBuilder()
                .setColor("#ED4245")
                .setDescription("You don't have permission to Manage Server."),
            ],
          });
        }
        ...
        try {
          ...
          await interaction.editReply({
            embeds: [
              new EmbedBuilder()
                .setColor("#57F287")
                .setDescription(`Announcement sent successfully to <#${targetChannel.id}>.`),
            ],
          });
        } catch (e) {
          await interaction.editReply({
            embeds: [
              new EmbedBuilder()
                .setColor("#ED4245")
                .setDescription(`Failed to send announcement: ${e.message}`),
            ],
          });
        }
  },
