...
function helpEmbed(client, cat) {
  ...
  if (!cat) {
    const e = (new client.ebuilder).setColor("#5865F2").setTitle("Feature Explorer").setDescription("Pick a category to explore commands.\n\n" + cats.map(c => `${categoryEmoji(c)} **${c}** — ${map.get(c).length} command(s)`).join("\n")).setFooter({
      text: `${cats.length} categories • ${[ ...map.values() ].flat().length} commands`
    });
    return e;
  }
  const list = map.get(cat) || [];
  const e = (new client.ebuilder).setColor("#5865F2").setTitle(`${categoryEmoji(cat)} ${cat}`).setDescription(list.length ? list.map(c => {
  ...
  }).join("\n") : "No commands.").setFooter({
    text: `/${cat} • ${list.length} command(s)`
  });
  return e;
}
...
function categoryRow(client, selected) {
  ...
  const menu = (new client.mbuilder).setCustomId("help_cat").setPlaceholder("Choose a category").addOptions(cats.map(c => ({
    label: c,
    value: c,
...
function backRow(client) {
  return (new client.abuilder).addComponents((new client.bbuilder).setCustomId("help_back").setLabel("Back").setStyle(client.ButtonStyle.Secondary));
}
...
async function execute(interaction) {
  ...
  col.on("collect", async i => {
    if (i.customId === "help_cat") {
      ...
      await i.update({
        embeds: [ e ],
        components: [ categoryRow(i.client, cat), backRow(i.client) ]
      }).catch(() => {});
    } else if (i.customId === "help_back") {
      const e = helpEmbed(i.client, null);
      await i.update({
        embeds: [ e ],
        components: [ categoryRow(i.client, null) ]
      }).catch(() => {});
    }
  });
  col.on("end", () => {
