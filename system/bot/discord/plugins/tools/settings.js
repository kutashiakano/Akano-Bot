...
async function apply(interaction, db, { nick: nick, avatar: avatar, banner: banner, bio: bio }, guildId) {
  ...
  if (!isAdmin) {
    return interaction.reply({
      embeds: [ (new interaction.client.ebuilder).setColor("#ED4245").setDescription("Only members with `Manage Server` or `Administrator` can change the settings.") ],
      flags: 64
    });
  }
  ...
  if (nick) {
    if (nick.length > 32) {
      return interaction.followUp({
        embeds: [ (new interaction.client.ebuilder).setColor("#ED4245").setDescription("Nickname is too long (max 32 characters).") ],
        flags: 64
      });
    }
    body.nick = nick;
  ...
  if (avatar) {
    ...
    if (!dataUri) {
      return interaction.followUp({
        embeds: [ (new interaction.client.ebuilder).setColor("#ED4245").setDescription("Avatar must be an image file (PNG/JPG/GIF).") ],
        flags: 64
      });
    }
    body.avatar = dataUri;
  ...
  if (banner) {
    ...
    if (!dataUri) {
      return interaction.followUp({
        embeds: [ (new interaction.client.ebuilder).setColor("#ED4245").setDescription("Banner must be an image file (PNG/JPG/GIF).") ],
        flags: 64
      });
    }
    body.banner = dataUri;
  ...
  if (bio) {
    if (bio.length > 190) {
      return interaction.followUp({
        embeds: [ (new interaction.client.ebuilder).setColor("#ED4245").setDescription("Bio is too long (max 190 characters).") ],
        flags: 64
      });
    }
    body.bio = bio;
  ...
  database.write(db);
  return interaction.followUp({
    embeds: [ (new interaction.client.ebuilder).setColor("#57F287").setTitle("Settings Updated").setDescription("Applied to **" + (interaction.guild?.name || "this server") + "**:\n\n" + changed.join("\n")).setTimestamp() ],
    flags: 64
  });
}
  ...
  run: async ctx => {
    ...
    try {
      ...
      if (!nick && !avatar && !banner && !bio) {
        return (await import("./status.js")).view(interaction);
      }
      return await apply(interaction, db, {
        nick: nick,
        avatar: avatar,
        banner: banner,
        bio: bio
      ...
      try {
        return interaction.reply({
          embeds: [ (new interaction.client.ebuilder).setColor("#ED4245").setDescription("Sorry, an error occurred while reading the settings.").setTimestamp() ],
          flags: 64
        });
      } catch (err) {}
    }
