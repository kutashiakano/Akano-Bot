...
async function view(interaction) {
  ...
  const embed = (new interaction.client.ebuilder).setColor("#5865F2").setTitle(guild.name).setThumbnail(guild.iconURL({
    dynamic: true,
    size: 256
  }) || null).setImage(guild.bannerURL({
    size: 1024
  }) || null).addFields({
    name: "Owner",
    value: `<@${guild.ownerId}>`,
    ...
    name: "Created",
    value: `<t:${Math.floor(guild.createdTimestamp / 1e3)}:D>`,
    inline: true
  }, {
    name: "Members",
    value: humans !== null ? `Total: **${guild.memberCount}**\nManusia: **${humans}** | Bot: **${bots}**` : `Total: **${guild.memberCount}**`,
    inline: true
  }, {
    name: "Channels",
  ...
  if (isAdmin) {
    ...
    const errLine = disabledPlugins.length ? disabledPlugins.map(n => "`" + n + "`").join(", ") : "None — all commands healthy";
    ...
    embed.addFields({
      name: "Commands Disabled Due To Errors",
      value: errLine,
      inline: true
    }, {
      name: "Bot Nickname (this server)",
      value: nickLine,
      inline: true
    }, {
      name: "Bot Bio (this server)",
      value: bioLine,
      inline: true
    });
  }
...
module.exports = define({
  ...
  run: async function(c) {
    return __orig.execute.call(__orig, c.interaction);
  }
});