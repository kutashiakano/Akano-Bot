...
module.exports = define({
  ...
  run: async ctx => {
    ...
    try {
      ...
      const chunks = splitMessage(fullResponse, 4e3);
      const firstEmbed = (new EmbedBuilder).setColor("#4285F4").setAuthor({
        name: "Gemini AI",
      ...
      await interaction.editReply({
        embeds: [ (new EmbedBuilder).setColor("#ED4245").setDescription("Gemini is currently unavailable. Please try again later.") ]
      });
    }
  }
});