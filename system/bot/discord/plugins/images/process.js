const {Jimp: Jimp, measureText: measureText, measureTextHeight: measureTextHeight} = require("jimp");
const path = require("path");
const fs = require("fs");
const fetch = require("node-fetch");
const {loadBitmapFontData: loadBitmapFontData} = require(path.join(path.dirname(require.resolve("@jimp/plugin-print")), "load-bitmap-font.js"));

const FONT_PATH = path.join(path.dirname(require.resolve("@jimp/plugin-print")), "../../fonts/open-sans/open-sans-32-white/open-sans-32-white");
...
async function getMemeFont() {
  ...
  const pngDataUrl = "data:image/png;base64," + fs.readFileSync(FONT_PATH + ".png").toString("base64");
   ...
   case "meme":
    {
      ...
      if (textTop) {
        const barH = Math.min(Math.round(img.height * .4), Math.round(measureTextHeight(font, textTop, wrapWidth) * 1.2 + 10));
        const bar = new Jimp({
          width: img.width,
          height: barH,
          color: 255
      ...
      if (textBottom) {
        const barH = Math.min(Math.round(img.height * .4), Math.round(measureTextHeight(font, textBottom, wrapWidth) * 1.2 + 10));
        const bar = new Jimp({
          width: img.width,
          height: barH,
          color: 255
