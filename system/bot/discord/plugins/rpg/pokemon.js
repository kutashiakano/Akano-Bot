...
module.exports = define({
  ...
  run: async ctx => {
    ...
    if (!q) return interaction.editReply({ content: "Usage: /pokemon <name>" }).catch(() => {});
    try {
      ...
      const e = new EB().setColor("#EE8130").setTitle("#" + data.id + " " + title(data.name)).setDescription(flavor.slice(0, 300) || "No description.").addFields(
      ...
      return interaction.editReply({ embeds: [new EB().setColor("#ED4245").setDescription("No results for `" + q + "`")] }).catch(() => {});
    }
  }
});
