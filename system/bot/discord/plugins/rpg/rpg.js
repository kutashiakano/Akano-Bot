...
function icon(name) {
  ...
  try {
    const buf = fs.readFileSync(path.join(ICON_DIR, name + ".png"));
    iconCache[name] = buf;
    return buf;
  } catch (e) {
    return null;
...
function art(name, embed) {
  const buf = icon(name);
  if (buf) {
    embed.setThumbnail("attachment://" + name + ".png");
    return [{ attachment: buf, name: name + ".png" }];
  }
  return [];
}
const cd = new Map;
...
const ACT = {
  async profile(EB, id, name, targetId, targetName) {
    ...
    const e = new EB().setColor("#5865F2").setTitle(who + " — Lv." + lvl + " " + titleOf(u)).setDescription(bar(u.rpg.xp - (lvl - 1) * 300, 300, 10) + " XP\n" + bar(u.rpg.hp, cap, 10) + " HP").addFields(
  ...
  async daily(EB, id, name) {
    ...
    me.rpg.hp = maxHp(me);
    await database.write(db).catch(() => {});
    return { embed: new EB().setColor("#57F287").setTitle("Daily Reward").setDescription("Received 500 money and 50 XP." + levelLine(g) + " HP fully restored. Balance: " + me.money + "."), art: "daily" };
  },
  async weekly(EB, id, name) {
    const db = database.get();
    const me = getU(db, id, name);
    if (Date.now() - (me.rpg.lastWeekly || 0) < 604800000) return { text: "Weekly reward already claimed. Come back in " + fmtTime(me.rpg.lastWeekly + 604800000 - Date.now()) + "." };
  ...
  async beg(EB, id, name) {
    ...
    gainXp(me, 5);
    await database.write(db).catch(() => {});
    return { embed: new EB().setColor("#57F287").setTitle("Begging").setDescription(line + " Received " + coins + " money and 5 XP."), art: "beg" };
  },
  async hunt(EB, id, name) {
    const db = database.get();
    const me = getU(db, id, name);
    ...
    if (powerOf(me) >= mob[1] * 8 + Math.random() * 15) {
      ...
      await database.write(db).catch(() => {});
      return { embed: new EB().setColor("#57F287").setTitle("Hunt — " + mob[0] + " defeated").setDescription("Loot: +" + gold + " money, +" + xp + " XP." + levelLine(g) + " You took " + dmg + " damage."), art: "hunt" };
    }
    ...
    gainXp(me, 5);
    await database.write(db).catch(() => {});
    return { faint: fainted(me), embed: new EB().setColor("#ED4245").setTitle("Hunt — escaped from " + mob[0]).setDescription("Lost " + lost + " money and took " + dmg + " damage. Consolation: +5 XP." + (fainted(me) ? " You fainted." : "")), art: "hunt" };
  },
  async fish(EB, id, name) {
    const db = database.get();
    const me = getU(db, id, name);
    ...
    await database.write(db).catch(() => {});
    return { embed: new EB().setColor("#57F287").setTitle("Fishing — caught " + pick[0]).setDescription("Sold for +" + gold + " money, +" + pick[3] + " XP." + levelLine(g)), art: "fish" };
  },
  async mine(EB, id, name) {
    const db = database.get();
    const me = getU(db, id, name);
    if (fainted(me)) return { text: "You have fainted. Recover first." };
    ...
    const g = gainXp(me, pick[3]);
    await database.write(db).catch(() => {});
    return { embed: new EB().setColor("#57F287").setTitle("Mining — struck " + pick[0]).setDescription("Sold for +" + gold + " money, +" + pick[3] + " XP." + levelLine(g)), art: "mine" };
  },
  async quest(EB, id, name) {
    const db = database.get();
    const me = getU(db, id, name);
    if (fainted(me)) return { faint: true, embed: new EB().setColor("#ED4245").setTitle("Fainted").setDescription("You have fainted. Drink a potion to recover."), art: "heal" };
    ...
    if (powerOf(me) >= boss) {
      ...
      await database.write(db).catch(() => {});
      return { embed: new EB().setColor("#57F287").setTitle("Quest — Dungeon Boss slain").setDescription("Grand loot: +" + gold + " money, +" + xp + " XP." + levelLine(g) + " You took " + dmg + " damage."), art: "boss" };
    }
    const dmg = hurt(me, 20 + Math.random() * 16);
    gainXp(me, 5);
    await database.write(db).catch(() => {});
    return { faint: fainted(me), embed: new EB().setColor("#ED4245").setTitle("Quest — defeated by the Dungeon Boss").setDescription("You took " + dmg + " damage and retreated with +5 XP." + (fainted(me) ? " You fainted." : "")), art: "boss" };
  },
  async heal(EB, id, name) {
    const db = database.get();
    const me = getU(db, id, name);
    const r = drinkPotion(me);
    if (!r.ok) return { text: r.why };
    await database.write(db).catch(() => {});
    return { embed: new EB().setColor("#57F287").setTitle("Healed").setDescription("Restored to " + r.hp + "/" + r.maxHp + " HP. Potions left: " + r.left + "."), art: "heal" };
  },
  async open(EB, id, name) {
    const db = database.get();
    const me = getU(db, id, name);
    ...
    await database.write(db).catch(() => {});
    return { embed: new EB().setColor("#FEE75C").setTitle("Loot Crate Opened").setDescription(result + " Crates left: " + me.rpg.crates + "."), art: "crate", again: me.rpg.crates > 0 };
  },
  async inventory(EB, id, name, targetId, targetName) {
    const db = database.get();
    const u = targetId && targetId !== id ? getU(db, targetId, targetName) : getU(db, id, name);
    const who = targetId && targetId !== id ? (targetName || "Player") : name;
    const pet = u.rpg.pet ? u.rpg.pet.name + " (+" + u.rpg.pet.power + " power, +" + u.rpg.pet.hp + " HP)" : "None";
    const e = new EB().setColor("#5865F2").setTitle(who + " — Inventory").addFields(
  ...
  async leaderboard(EB) {
    ...
    return { embed: new EB().setColor("#5865F2").setTitle("Top Hunters").setDescription(desc), art: "leaderboard" };
  }
};
...
module.exports = define({
  ...
  run: async ctx => {
    ...
    const attachHeal = async (embed, artName, userId) => {
      await send(embed, artName, [row(client, [btn(client, "rpg_heal", "Drink Potion (60 HP)", client.ButtonStyle.Success)])]);
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => i.customId === "rpg_heal" && i.user.id === userId && !i.user.bot, 120000, strip);
      if (!col) return;
      col.on("collect", async i => {
        ...
        const e2 = new EB().setColor("#57F287").setTitle("Healed").setDescription("Restored to " + r.hp + "/" + r.maxHp + " HP. Potions left: " + r.left + ".");
        const files = art("heal", e2);
    ...
    const renderMenu = async () => {
      ...
      const cap = maxHp(me);
      const e = new EB().setColor("#5865F2").setTitle("Adventure Menu — " + uname).setDescription("Lv." + levelOf(me.rpg.xp) + " " + titleOf(me) + " | " + me.money + " money | HP " + me.rpg.hp + "/" + cap + "\nTap a button to play. No typing needed.");
      const rows = MENU_ROWS.map(ids => row(client, ids.map(k => btn(client, "rpg_m_" + k, MENU_LABELS[k], client.ButtonStyle.Secondary))));
      await send(e, "profile", rows);
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => (i.customId.indexOf("rpg_m_") === 0 || i.customId === "rpg_shop_potion" || i.customId === "rpg_shop_sword" || i.customId === "rpg_shop_shield" || i.customId === "rpg_shop_crate" || i.customId === "rpg_shop_egg" || i.customId === "rpg_open_again") && i.user.id === uid && !i.user.bot, 300000, strip);
      if (!col) return;
      const backRow = [row(client, [btn(client, "rpg_m_menu", "Menu", client.ButtonStyle.Secondary)])];
      const showMenu = async i => {
        const d2 = database.get();
        const u2 = getU(d2, uid, uname);
        const cap2 = maxHp(u2);
        const e2 = new EB().setColor("#5865F2").setTitle("Adventure Menu — " + uname).setDescription("Lv." + levelOf(u2.rpg.xp) + " " + titleOf(u2) + " | " + u2.money + " money | HP " + u2.rpg.hp + "/" + cap2 + "\nTap a button to play. No typing needed.");
        const files = art("profile", e2);
        try { await i.update({ embeds: [e2], files: files, components: MENU_ROWS.map(ids => row(client, ids.map(k => btn(client, "rpg_m_" + k, MENU_LABELS[k], client.ButtonStyle.Secondary)))) }); } catch (e3) {}
      };
      const showRes = async (i, res, extraRows) => {
        if (res.text) {
          const e2 = new EB().setColor("#99AAB5").setDescription(res.text);
          try { await i.update({ embeds: [e2], components: backRow }); } catch (e3) {}
          return;
        }
        const files = art(res.art, res.embed);
      ...
      col.on("collect", async i => {
        ...
        if (key === "rpg_open_again") {
          const res = await ACT.open(EB, uid, uname);
          await showRes(i, res, res.again ? [row(client, [btn(client, "rpg_open_again", "Open Again", client.ButtonStyle.Primary)])] : []);
          return;
        }
        ...
        if (act === "shop") {
          const d2 = database.get();
          const u2 = getU(d2, uid, uname);
          const e2 = new EB().setColor("#5865F2").setTitle("Item Shop").setDescription("Balance: " + (u2.money || 0) + " money.").addFields(
          ...
          const files = art("shop", e2);
          try {
            await i.update({ embeds: [e2], files: files, components: [row(client, [btn(client, "rpg_shop_potion", "Potion", client.ButtonStyle.Success), btn(client, "rpg_shop_sword", "Sword", client.ButtonStyle.Primary), btn(client, "rpg_shop_shield", "Shield", client.ButtonStyle.Secondary)]), row(client, [btn(client, "rpg_shop_crate", "Crate", client.ButtonStyle.Primary), btn(client, "rpg_shop_egg", "Egg", client.ButtonStyle.Secondary), btn(client, "rpg_m_menu", "Menu", client.ButtonStyle.Secondary)])] });
          } catch (e2) {}
          return;
        }
        if (!ACT[act]) return;
        const res = await ACT[act](EB, uid, uname);
        await showRes(i, res, res.again ? [row(client, [btn(client, "rpg_open_again", "Open Again", client.ButtonStyle.Primary)])] : []);
      });
    };
    const runShop = async () => {
      const db = database.get();
      const me = getU(db, uid, uname);
      const e = new EB().setColor("#5865F2").setTitle("Item Shop").setDescription("Balance: " + (me.money || 0) + " money. Tap a button to buy one, or use /rpg buy for bulk amounts.").addFields(
      ...
      await send(e, "shop", [row(client, [btn(client, "rpg_shop_potion", "Potion " + POTION_COST, client.ButtonStyle.Success), btn(client, "rpg_shop_sword", "Sword", client.ButtonStyle.Primary), btn(client, "rpg_shop_shield", "Shield", client.ButtonStyle.Secondary)]), row(client, [btn(client, "rpg_shop_crate", "Crate " + CRATE_COST, client.ButtonStyle.Primary), btn(client, "rpg_shop_egg", "Egg " + EGG_COST, client.ButtonStyle.Secondary)])]);
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => i.customId.indexOf("rpg_shop_") === 0 && !i.user.bot, 120000, strip);
      if (!col) return;
      col.on("collect", async i => {
    ...
    if (sub === "profile") {
      ...
      const res = target.id === uid ? await ACT.profile(EB, uid, uname) : await ACT.profile(EB, uid, uname, String(target.id), target.username);
      await sendRes(res);
      return;
    }
    if (sub === "daily") {
    ...
    if (sub === "weekly") {
      await sendRes(await ACT.weekly(EB, uid, uname));
      return;
    }
    if (sub === "work") {
      await sendRes(await ACT.work(EB, uid, uname));
      return;
    }
    if (sub === "beg") {
      await sendRes(await ACT.beg(EB, uid, uname));
      return;
    }
    if (sub === "hunt") {
      const res = await ACT.hunt(EB, uid, uname);
      if (res.faint) {
    ...
    if (sub === "fish") {
      await sendRes(await ACT.fish(EB, uid, uname));
      return;
    }
    if (sub === "mine") {
      await sendRes(await ACT.mine(EB, uid, uname));
      return;
    }
    if (sub === "quest") {
      const res = await ACT.quest(EB, uid, uname);
      if (res.faint) {
        await attachHeal(res.embed, res.art, uid);
        return;
      }
    ...
    if (sub === "heal") {
      await sendRes(await ACT.heal(EB, uid, uname));
      return;
    }
    if (sub === "open") {
      const res = await ACT.open(EB, uid, uname);
      ...
      await send(res.embed, res.art, [row(client, [btn(client, "rpg_open_again", "Open Again", client.ButtonStyle.Primary)])]);
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => i.customId === "rpg_open_again" && i.user.id === uid && !i.user.bot, 60000, strip);
      if (!col) return;
      col.on("collect", async i => {
        const res2 = await ACT.open(EB, uid, uname);
        if (res2.text) {
          try { await i.reply({ content: res2.text, flags: 64 }); } catch (e2) {}
          try { col.stop(); } catch (e2) {}
          return;
        }
        const files = art("crate", res2.embed);
        const comps = res2.again ? [row(client, [btn(client, "rpg_open_again", "Open Again", client.ButtonStyle.Primary)])] : [];
    ...
    if (sub === "inventory") {
      const target = interaction.options.getUser("user") || interaction.user;
      const res = target.id === uid ? await ACT.inventory(EB, uid, uname) : await ACT.inventory(EB, uid, uname, String(target.id), target.username);
      await sendRes(res);
      return;
    }
    if (sub === "leaderboard") {
      await sendRes(await ACT.leaderboard(EB));
      return;
    }
    if (sub === "duel") {
      const db = database.get();
      ...
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => (i.customId === "rpg_duel_yes" || i.customId === "rpg_duel_no") && i.user.id === foe.id && !i.user.bot, 60000, strip);
      if (!col) return;
      col.on("collect", async i => {
        if (i.customId === "rpg_duel_no") {
          const e2 = new EB().setColor("#99AAB5").setTitle("Duel Declined").setDescription(foe.username + " declined the duel.");
        ...
        if (!isLinked(String(foe.id))) {
          const e2 = new EB().setColor("#ED4245").setTitle("Duel Cancelled").setDescription(foe.username + " must link their account with /register first.");
          try { await i.update({ embeds: [e2], components: [] }); } catch (e3) {}
          try { col.stop(); } catch (e3) {}
          return;
        }
    ...
    if (sub === "gamble") {
      ...
      const e = new EB().setColor("#FEE75C").setTitle("Coin Flip — " + bet + " on the table").setDescription(uname + ", pick a side. Winner takes " + bet * 2 + ".");
      await send(e, "gamble", [row(client, [btn(client, "rpg_gamble_h", "Heads", client.ButtonStyle.Primary), btn(client, "rpg_gamble_t", "Tails", client.ButtonStyle.Secondary)])]);
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => (i.customId === "rpg_gamble_h" || i.customId === "rpg_gamble_t") && i.user.id === uid && !i.user.bot, 30000, strip);
      if (!col) return;
      col.on("collect", async i => {
        const d2 = database.get();
        const u2 = getU(d2, uid, uname);
        ...
        if (pick === flip) {
          ...
          e2 = new EB().setColor("#57F287").setTitle("Coin Flip — " + flip + ", you win").setDescription("+" + bet + " money (balance " + u2.money + "), +15 XP." + levelLine(g));
        } else {
          u2.money = Math.max(0, u2.money - bet);
          e2 = new EB().setColor("#ED4245").setTitle("Coin Flip — " + flip + ", you lose").setDescription("-" + bet + " money (balance " + u2.money + ").");
        }
        await database.write(d2).catch(() => {});
        const files = art("gamble", e2);
        try { await i.update({ embeds: [e2], files: files, components: [] }); } catch (e3) {}
        try { col.stop(); } catch (e3) {}
      });
      return;
    ...
    if (sub === "slots") {
      ...
      if (s[0] === s[1] && s[1] === s[2]) {
        ...
        e = new EB().setColor("#57F287").setTitle("Slots — JACKPOT").setDescription("[ " + s.join(" | ") + " ] Triple match. +" + bet * 5 + " money (balance " + me.money + "), +25 XP." + levelLine(g));
      } else if (s[0] === s[1] || s[1] === s[2] || s[0] === s[2]) {
        me.money += bet * 2;
        const g = gainXp(me, 10);
        e = new EB().setColor("#57F287").setTitle("Slots — Pair").setDescription("[ " + s.join(" | ") + " ] Pair match. +" + bet * 2 + " money (balance " + me.money + "), +10 XP." + levelLine(g));
      } else {
        e = new EB().setColor("#ED4245").setTitle("Slots — No luck").setDescription("[ " + s.join(" | ") + " ] -" + bet + " money (balance " + me.money + ").");
      }
      await database.write(db).catch(() => {});
      await send(e, "slots");
      return;
    ...
    if (sub === "oracle") {
      ...
      await send(new EB().setColor("#9B59B6").setTitle("Oracle").setDescription("Question: " + q + "\nAnswer: " + a), "oracle");
      return;
    }
    if (sub === "fate") {
      ...
      await send(new EB().setColor("#9B59B6").setTitle("Fate Decides").setDescription("Between " + a + " and " + b + ", fate chooses " + pick + "."), "quest");
      return;
    }
    if (sub === "shop") {
      await runShop();
    ...
    if (sub === "buy") {
      ...
      await database.write(db).catch(() => {});
      await send(new EB().setColor("#57F287").setTitle("Purchase Complete").setDescription(r.text), "shop");
      return;
    }
  }
});
