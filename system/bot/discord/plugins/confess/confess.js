...
const ICON_DIR = path.join(__dirname, "..", "..", "..", "website", "dashboard", "client", "icons");
let maskBuf = null;
function maskFile() {
  if (maskBuf) return [{ attachment: maskBuf, name: "mask.png" }];
  try {
    maskBuf = fs.readFileSync(path.join(ICON_DIR, "mask.png"));
    return [{ attachment: maskBuf, name: "mask.png" }];
  } catch (e) {
    return [];
  }
}
function cfgOf(db, gid, gname) {
  ...
  if (!s.confess || typeof s.confess !== "object") s.confess = { channels: [], review: false, color: "#5865F2", logs: false, nextId: 1, banned: [], items: {}, pending: {}, authors: {} };
  const c = s.confess;
...
function confessEmbed(EB, c, id, text, footer) {
  return new EB().setColor(colorOf(c)).setAuthor({ name: "Anonymous Confession (#" + id + ")", iconURL: "attachment://mask.png" }).setDescription(String(text).slice(0, 1500)).setFooter({ text: footer || ("Report abuse with /confess report id:" + id) }).setTimestamp();
}
async function postConfession(client, channelId, embed, extra, replyToId) {
  const ch = await client.channels.fetch(channelId).catch(() => null);
  if (!ch || !ch.isTextBased()) return null;
  const files = maskFile();
  if (extra) files.push(extra);
  const comps = replyToId ? [row(client, [btn(client, "cf_reply_" + replyToId, "Reply", client.ButtonStyle.Primary), btn(client, "cf_poll", "Poll", client.ButtonStyle.Secondary)])] : [];
...
function btn(client, id, label, style) {
  return new client.bbuilder().setCustomId(id).setLabel(label).setStyle(style || client.ButtonStyle.Primary);
}
function row(client, buttons) {
  const r = new client.abuilder();
  r.addComponents(...buttons);
  return r;
}
async function watch(msg, filter, ms, onEnd) {
  let col = null;
  try {
    col = msg.createMessageComponentCollector({ filter: filter, time: ms });
  } catch (e) {
    return null;
  }
  if (onEnd) col.on("end", () => { try { onEnd(msg); } catch (e) {} });
  return col;
}
function strip(msg) {
  try { msg.edit({ components: [] }).catch(() => {}); } catch (e) {}
}
function queueEmbed(EB, c) {
  ...
  return new EB().setColor(colorOf(c)).setTitle("Moderation Queue").setDescription(lines.join("\n").slice(0, 1800));
}
function queueRows(client, c) {
  ...
  const mk = [];
  for (const p of pend) {
    mk.push(btn(client, "cf_ap_" + p.id, "Approve #" + p.id, client.ButtonStyle.Success));
    mk.push(btn(client, "cf_del_" + p.id, "Reject #" + p.id, client.ButtonStyle.Danger));
  }
  for (const x of rep) {
    mk.push(btn(client, "cf_keep_" + x.id, "Keep #" + x.id, client.ButtonStyle.Success));
    mk.push(btn(client, "cf_rm_" + x.id, "Remove #" + x.id, client.ButtonStyle.Danger));
  }
  while (mk.length) rows.push(row(client, mk.splice(0, 5)));
  return rows.slice(0, 5);
}
async function doReply(client, EB, gid, gname, uid, uname, id, text, mediaFile, toN) {
  ...
  const e = new EB().setColor(colorOf(c)).setAuthor({ name: "Anonymous Reply #" + id + "." + n + (quotedN ? " to #" + id + "." + quotedN : ""), iconURL: "attachment://mask.png" }).setDescription(text).setFooter({ text: footerOf(db, c, id, String(uid)) }).setTimestamp();
  const sendFiles = maskFile();
  if (mediaFile) sendFiles.push(mediaFile);
  const payload = { embeds: [e], files: sendFiles };
...
async function handleComponent(interaction) {
  ...
  const id = parseInt(cid.replace("cf_reply_", ""), 10);
  if (!id || isNaN(id)) return;
  const modal = new interaction.client.ModalBuilder().setCustomId("cf_modal_" + id).setTitle("Reply to #" + id);
...
async function handleModal(interaction) {
  ...
  let id = parseInt(mid.replace("cf_modal_", ""), 10);
  let text = "";
  try {
  ...
  const r = await doReply(interaction.client, EB, interaction.guildId, interaction.guild && interaction.guild.name, String(interaction.user.id), interaction.user.username, id, text, null, null);
  try {
...
module.exports = define({
  ...
  options: [
    ...
    { name: "reply", type: 1, description: "Reply anonymously to a confession", options: [{ name: "id", type: 4, description: "Confession number", required: true }, { name: "message", type: 3, description: "Your reply (posted anonymously)", required: true }, { name: "to", type: 4, description: "Quote a specific reply number, example 1 for #2.1", required: false }, { name: "media", type: 11, description: "Optional image or video to attach", required: false }] },
    ...
    { name: "settings", type: 1, description: "Your personal confession settings", options: [{ name: "initials", type: 3, description: "Initials shown in footer, example T (max 8)", required: false }, { name: "show", type: 3, description: "Show or hide your initials", required: false, choices: [{ name: "show", value: "on" }, { name: "hide", value: "off" }] }] },
    ...
    { name: "help", type: 1, description: "How to use confessions (only you see this)" }
  ],
  run: async ctx => {
    ...
    if (pre === "help") {
      const e = new EB().setColor("#5865F2").setAuthor({ name: "Confessions Guide", iconURL: "attachment://mask.png" }).setDescription("Everyone posts anonymously. Staff never see names unless author logging is ON (check with /confess checklogs).").addFields(
        { name: "Setup (staff)", value: "/confess channel add #curhat — set target channel\n/confess review on/off — pre-approve posts\n/confess color #9B59B6 — embed color", inline: false },
        { name: "Everyone", value: "/confess send — post with optional media, markdown supported\nReply button on any confession — one-field form, auto-targets and quotes latest\n/confess reply — full control: id, quote target to:, media file\n/confess report — 3 trusted reports hide a post for review\n/confess edit / delete — manage your own posts\n/confess settings — footer initials like by T\n/confess stats — server totals", inline: false },
        { name: "Staff", value: "/confess queue — approve, reject, keep, or remove with buttons\n/confess ban / unban", inline: false }
      );
      try { await interaction.reply({ embeds: [e], files: maskFile(), flags: 64 }); } catch (e2) {}
      return;
    }
    ...
    if (sub === "send") {
      ...
      const msg = await postConfession(client, c.channels[0], confessEmbed(EB, c, id, text, footerOf(db, c, id, String(interaction.user.id))), m1, id);
    ...
    if (sub === "reply") {
      ...
      const r = await doReply(client, EB, String(interaction.guildId), interaction.guild && interaction.guild.name, String(interaction.user.id), interaction.user.username, id, text, m2, toN);
      if (r.error) {
    ...
    if (sub === "report") {
      ...
      if (trusted >= 3 && !item.hidden) {
        ...
        try {
          ...
          if (orig) await orig.edit({ embeds: [new EB().setColor("#EAB308").setTitle("Confession #" + id).setDescription("This confession is under staff review.")] }).catch(() => {});
        } catch (e) {}
    ...
    if (sub === "stats") {
      ...
      const e = new EB().setColor(colorOf(c)).setAuthor({ name: "Confession Stats", iconURL: "attachment://mask.png" }).addFields(
    ...
    if (sub === "delete") {
      ...
      try {
        const ch = await client.channels.fetch(item.channelId).catch(() => null);
        const orig = ch && item.messageId ? await ch.messages.fetch(item.messageId).catch(() => null) : null;
        if (orig) await orig.edit({ embeds: [new EB().setColor("#99AAB5").setTitle("Confession #" + id).setDescription("This confession was deleted.")] }).catch(() => {});
      } catch (e) {}
      await save();
    ...
    if (sub === "edit") {
      ...
      try {
        const ch = await client.channels.fetch(item.channelId).catch(() => null);
        const orig = ch && item.messageId ? await ch.messages.fetch(item.messageId).catch(() => null) : null;
        if (orig) await orig.edit({ embeds: [confessEmbed(EB, c, id, text, footerOf(db, c, id, c.authors[id]))] }).catch(() => {});
      } catch (e) {}
      await save();
    ...
    if (sub === "settings") {
      ...
      await interaction.editReply({ content: "Your confession settings: " + state + ". Markdown like bold, italic, and spoiler works in confessions." }).catch(() => {});
      return;
    }
    ...
    const configPanel = (cc) => {
      ...
      return new EB().setColor(colorOf(src)).setAuthor({ name: "Server Configuration", iconURL: "attachment://mask.png" }).addFields(
        { name: "Channels", value: "Confessions: " + list, inline: false },
    ...
    if (sub === "config") {
      const e = configPanel();
      const files = maskFile();
      await interaction.editReply({ embeds: [e], files: files, components: [row(client, [btn(client, "cf_cfg_rev", "Review: " + (c.review ? "ON" : "OFF"), client.ButtonStyle.Primary), btn(client, "cf_cfg_log", "Logs: " + (c.logs ? "ON" : "OFF"), client.ButtonStyle.Secondary), btn(client, "cf_cfg_color", "Set Color", client.ButtonStyle.Secondary)])] }).catch(() => {});
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => i.customId.indexOf("cf_cfg_") === 0 && i.user.id === String(interaction.user.id) && isMod(i) && !i.user.bot, 300000, strip);
      if (!col) return;
      col.on("collect", async i => {
        ...
        if (i.customId === "cf_cfg_rev") {
          ...
          const r2 = [row(client, [btn(client, "cf_cfg_rev", "Review: " + (c2.review ? "ON" : "OFF"), client.ButtonStyle.Primary), btn(client, "cf_cfg_log", "Logs: " + (c2.logs ? "ON" : "OFF"), client.ButtonStyle.Secondary), btn(client, "cf_cfg_color", "Set Color", client.ButtonStyle.Secondary)])];
    ...
    if (sub === "setup") {
      ...
      const step1 = new EB().setColor(colorOf(c)).setAuthor({ name: "Confessions Setup", iconURL: "attachment://mask.png" }).setTitle("Step 1/4 — Confessions Channel").setDescription("Select one or more channels where confessions will be posted.");
      ...
      const cancelRow = row(client, [btn(client, "cf_setup_cancel", "Cancel", client.ButtonStyle.Danger)]);
      const files = maskFile();
      await interaction.editReply({ embeds: [step1], files: files, components: [row(client, [chSel]), cancelRow] }).catch(() => {});
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => (i.customId === "cf_setup_ch" || i.customId === "cf_setup_cancel" || i.customId === "cf_setup_rev_on" || i.customId === "cf_setup_rev_off" || i.customId === "cf_setup_log_on" || i.customId === "cf_setup_log_off" || i.customId === "cf_setup_done") && i.user.id === authorId && isMod(i) && !i.user.bot, 300000, strip);
      if (!col) return;
      col.on("collect", async i => {
        const d2 = database.get();
        const c2 = cfgOf(d2, String(i.guildId), i.guild && i.guild.name);
        ...
        if (i.customId === "cf_setup_cancel") {
          try { await i.update({ embeds: [new EB().setColor("#99AAB5").setTitle("Setup Cancelled").setDescription("Nothing was changed.")], components: [] }); } catch (e2) {}
        ...
        if (i.customId === "cf_setup_ch") {
          ...
          c2.channels = valid;
          await database.write(d2).catch(() => {});
          const e2 = new EB().setColor(colorOf(c2)).setAuthor({ name: "Confessions Setup", iconURL: "attachment://mask.png" }).setTitle("Step 2/4 — Review Channel").setDescription("Channels saved: " + valid.map(x => "<#" + x + ">").join(" ") + "\n\nReview confessions before they are posted?");
          await again(e2, [row(client, [btn(client, "cf_setup_rev_on", "Review ON", client.ButtonStyle.Success), btn(client, "cf_setup_rev_off", "Post Instantly", client.ButtonStyle.Primary)]), cancelRow]);
          return;
        }
        if (i.customId === "cf_setup_rev_on" || i.customId === "cf_setup_rev_off") {
          c2.review = i.customId === "cf_setup_rev_on";
          await database.write(d2).catch(() => {});
          const e2 = new EB().setColor(colorOf(c2)).setAuthor({ name: "Confessions Setup", iconURL: "attachment://mask.png" }).setTitle("Step 3/4 — Author Logs").setDescription("Review mode " + (c2.review ? "ON. Approve posts via /confess queue." : "OFF.") + "\n\nLog author IDs for posted confessions? Users are always told via /confess checklogs.");
          await again(e2, [row(client, [btn(client, "cf_setup_log_on", "Logs ON", client.ButtonStyle.Success), btn(client, "cf_setup_log_off", "Logs OFF", client.ButtonStyle.Primary)]), cancelRow]);
          return;
        }
        if (i.customId === "cf_setup_log_on" || i.customId === "cf_setup_log_off") {
          c2.logs = i.customId === "cf_setup_log_on";
          await database.write(d2).catch(() => {});
          const e2 = new EB().setColor(colorOf(c2)).setAuthor({ name: "Confessions Setup", iconURL: "attachment://mask.png" }).setTitle("Step 4/4 — Done").setDescription("Confessions: " + (c2.channels.length ? c2.channels.map(x => "<#" + x + ">").join(" ") : "None") + "\nReview: " + (c2.review ? "ON" : "OFF") + "\nAuthor Logs: " + (c2.logs ? "ON" : "OFF") + "\nColor: " + colorOf(c2) + "\n\nMembers can now use /confess send. Tune anytime via /confess config.");
          await again(e2, []);
          try { col.stop(); } catch (e2) {}
          return;
        }
    ...
    if (sub === "queue") {
      const e = queueEmbed(EB, c);
      const rows = queueRows(client, c);
      await interaction.editReply({ embeds: [e], components: rows }).catch(() => {});
      if (!rows.length) return;
      const msg = await freshMsg();
      if (!msg) return;
      const col = await watch(msg, i => i.customId.indexOf("cf_") === 0 && isMod(i) && !i.user.bot, 300000, strip);
      if (!col) return;
      col.on("collect", async i => {
        ...
        if (act === "ap") {
          ...
          const posted = await postConfession(client, (c2.channels[0] || null), confessEmbed(EB, c2, id, p.text, footerOf(d2, c2, id, c2.authors[id])), p.media ? { attachment: p.media.url, name: p.media.name } : null, id);
          ...
          if (x) {
            ...
            if (x.hidden) {
              ...
              try {
                ...
                if (orig) await orig.edit({ embeds: [confessEmbed(EB, c2, id, x.text, footerOf(d2, c2, id, c2.authors[id]))] }).catch(() => {});
              } catch (e3) {}
            }
          }
        ...
        if (act === "rm") {
          ...
          if (x) {
            ...
            try {
              const ch = await client.channels.fetch(x.channelId).catch(() => null);
              const orig = ch && x.messageId ? await ch.messages.fetch(x.messageId).catch(() => null) : null;
              if (orig) await orig.edit({ embeds: [new EB().setColor("#ED4245").setTitle("Confession #" + id).setDescription("This confession was removed by a server moderator.")] }).catch(() => {});
            } catch (e3) {}
          }
    ...
    if (sub === "approve" || sub === "reject") {
      ...
      delete c.pending[id];
      if (sub === "approve") {
        const posted = await postConfession(client, (c.channels[0] || null), confessEmbed(EB, c, id, p.text, footerOf(db, c, id, c.authors[id])), p.media ? { attachment: p.media.url, name: p.media.name } : null, id);
