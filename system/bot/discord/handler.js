...
module.exports = {
  setup(client) {
    ...
    client.on("interactionCreate", async interaction => {
      try {
        require("fs").appendFileSync(process.env.HOME + "/.akano-debughandler.log", JSON.stringify({
      ...
      if (!message.guild && message.channel?.type === 1) {
        try {
          ...
          if (!user?.registered) {
            ...
            const {ActionRowBuilder: ActionRowBuilder, StringSelectMenuBuilder: StringSelectMenuBuilder} = require("discord.js");
